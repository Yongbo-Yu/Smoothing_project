...
\begin{table}[!h]
	...
	\caption{ In this table, we highlight the computational gains achieved by ASGQ and QMC over MC method to meet a certain error tolerance. We note that the ratios are computed \red{for the best configuration with Richardson extrapolation for each method}.}
		\begin{tabular}{l*{4}{c}r}
			\toprule[1.5pt]
			\textbf{Parameters}              &  \textbf{Relative error}  & \textbf{CPU time ratio $\left(\text{MC}/ \text{ASGQ} \right)$} & \textbf{CPU time ratio  $\left(\text{MC}/ \text{QMC} \right)$}\\
			\hline
			Set $1$  &  $1\%$&  $ 15$ &  $10$\\	
...
\begin{frame}[plain]\frametitle{\centerline{Complexity of the MC method}
...
\begin{figure}
	...
	\caption{Numerical complexity of the MC  method with the different configurations in terms of  Richardson extrapolation 's level.  Case of \red{parameter set $1$ in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}}. }
	\label{fig: Comparing the numerical complexity of the MC  method with the different configurations}
\end{figure}
\end{frame}


\begin{frame}[plain]\frametitle{\centerline{Complexity of the QMC method}
\centerline{with different configurations }}
\begin{figure}
		\centering
		\includegraphics[width=0.7\textwidth]{./rBergomi_Complexity_rates/set2/error_vs_time_set2_QMC_comparison}
	\caption{Numerical complexity of the QMC  method with the different configurations in terms of  Richardson extrapolation 's level.  Case of \red{parameter set $1$ in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}}. }
	\label{fig: Comparing the numerical complexity of the QMC  method with the different configurations}
\end{figure}
\end{frame}


\begin{frame}[plain]\frametitle{\centerline{Complexity of the ASGQ method}
\centerline{with different configurations }}
\begin{figure}
		\centering
...
\begin{figure}[h!]
	...
	\caption{Computational work comparison for the different methods \red{with the best configurations concluded from Figures \ref{fig: Comparing the numerical complexity of the MC  method with the different configurations}, \ref{fig: Comparing the numerical complexity of the QMC  method with the different configurations} and \ref{fig: Comparing the numerical complexity of the ASGQ  method with the different configurations}}, for the case of \red{parameter set $1$ in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}}.}	\label{fig:Complexity plot for  MISC for Case set $2$ parameters, comparison}
\end{figure}
\end{frame}
