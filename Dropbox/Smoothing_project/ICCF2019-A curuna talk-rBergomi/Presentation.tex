...
\item \textbf{Numerically:}
	\begin{itemize}
		...
		\red{vanilla options} is \red{Monte Carlo (MC)} \cite   {bayer2016pricing,mccrickerd2018turbocharging}: still a \red{time consuming task}.
...
\begin{frame}[plain]\frametitle{\centerline{Methodology \cite{bayer2018hierarchical}}}
 ...
 We design a \red{hierarchical efficient pricing method} based on
		\begin{enumerate}
			...
			\end{itemize}			  
			\item Coupling our methods with \red{hierarchical representations} $\Rightarrow$ \red{Reduce the dimension} of the problem.
			\begin{itemize}
			\item \textbf{Brownian bridges} as a Wiener path generation method.
			\item \textbf{Richardson Extrapolation} (\red{Condition: weak error of order $1$)} 
		$\quad\Rightarrow$ Faster convergence of the weak error  $\Rightarrow $ $\searrow$ number of time steps (smaller dimension).
		\end{itemize}
		\end{enumerate} 
...
\begin{frame}[plain]	
	\frametitle{\centerline{Sparse grids I \cite{bungartz2004sparse} }}
...
\begin{frame}[plain,shrink=4]	
	\frametitle{\centerline{Sparse grids II \cite{bungartz2004sparse}}}
	\vspace{0.3cm}
...
\item \red{Regular SG}: $ \mathcal{I}_{\ell}=\{  \mid \boldsymbol{\beta} \mid_1 \le \ell+d-1; \: \boldsymbol{\beta} \in \mathbb{N}^d_{+} \} $
 \begin{figure}
      \begin{columns}
        \column{.5\linewidth}
        \includegraphics[scale=0.4]{sparse_grids}
        \column{.4\linewidth}
...
\begin{itemize}
\item Set $1$ is the  \red{closest to the empirical findings} \cite{gatheral2018volatility}, suggesting that $\blue{H} \approx 0.1$. The choice $\blue{\nu}= 1.9$ and $\blue{\rho}=-0.9$ is justified by \cite{bayer2016pricing}.
