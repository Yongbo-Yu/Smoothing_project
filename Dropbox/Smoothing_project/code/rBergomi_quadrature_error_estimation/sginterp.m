function [Y_out,Z,Y]=sginterp(sg_struct,X)
    ...
    Y=zeros(size(I,1),size(X,1),2);
    Z=zeros(size(I,1),size(X,1),2);
            ...
            for k=1:1:size(X,1),
                ...
                for d=1:1:length(x_j)
                    if sg_struct.scheme(d)=='C'
                        lb=lb*lagrangeb(knots(d,idx(d),1:M_i(d)),x_k(d),x_j(d));
                    end
