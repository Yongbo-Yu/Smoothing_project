...
range = max(6,floor(0.4*length(L))):length(L);
...
% subplot(nvert,2,1)
plot(L,log2(var2),L(2:end),log2(var1(2:end)),L,-2+log2(2.^(-L)), L(6:end),polyval(pb,L(6:end)),'-r')
% hold on
...
abs(del2)
abs(del1(2:end))
plot(L,log2(abs(del2)),L(2:end),log2(abs(del1(2:end))),L,log2(2.^(-L)),L(2:end),polyval(pa,L(2:end)),'-r')
