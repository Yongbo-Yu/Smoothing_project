# In this file, we plot the weak convergence rate for MC with Richardson extrapolation (level1) after doing the partial change of measure (corrected version)
...
# attributes
   ...
   # y1perp = Vector(N)
    ...
    def fun(self,y11):       
        ...
        y=np.array([y11,yperp]).reshape(4*Nsteps)
    ...
    def objfun(self,Nsteps):
        ...
        if Nsteps<=2:
            ...
            bar_y_f=np.sqrt(2)*np.dot(self.L,y[0:4])+self.z_bar
            #bar_y_c=bar_y_f[0:2]
            bar_y_c=bar_y_f
            ...
            if self.N==4:
                ...
                cst_f=np.linalg.det(self.L)*(2**(self.N/2))*(np.exp(0.5*y[0:4].dot(y[0:4])))
                ...
                cst_f=np.linalg.det(self.L)*(2**(Nsteps/4))*(np.exp(0.5*y[0:4].dot(y[0:4])))
