...
#methods
    ...
    def __init__(self,nested=False):
        ...
        self.z_bar=np.zeros((2*self.N,1))
        from scipy.optimize import minimize
        mini = minimize(self.fun2, self.z_bar, method='powell',  options={'xtol': 1e-5, 'disp': False})
        self.z_bar=mini.x
    ...
    def objfun(self,y):
        ...
        bar_y=np.sqrt(2)*np.dot(self.L,y)+self.z_bar
       
        #hierarchical
        yperp_1=bar_y[self.N+1:2*self.N]
        yperp1=bar_y[self.N]
        bbperp=self.brownian_increments(yperp1,yperp_1)
        W1perp= [(bbperp[i+1]-bbperp[i]) *np.sqrt(self.N) for i in range(0,len(bbperp)-1)]
        ...
        cst=np.linalg.det(self.L)*(2**self.N)*(np.exp(0.5*y.dot(y)))
        QoI=(self.z.ComputePayoffRT_single(W1,W1perp))*cst*(np.exp(-0.5*(bar_y).dot(bar_y)))
...
def first_difference_rate_plotting():       
    ...
    plt.savefig('./results/first_difference_rbergomi_4steps_H_007_K_1_totally_hierarch_with_rate_W2_change_measure_tot.eps', format='eps', dpi=1000)
...
def mixed_difference_order2_rate_plotting(d):       
    ...
    for k in range(4,8,1):  
       ...
       # QoI_beta=np.zeros(prb.N,dtype=int)
        ...
        plt.ylabel(r'$\mid \Delta E_{\mathbf{1}+k \bar{\beta}} \mid $',fontsize=14)  
    plt.legend(loc='lower left')
    plt.savefig('./results/mixed_difference_order2_rbergomi_4steps_H_007_K_1_totally_hierarch_with_rate_W2_change_measure_tot.eps', format='eps', dpi=1000)       
    
    


first_difference_rate_plotting()
mixed_difference_order2_rate_plotting(4)