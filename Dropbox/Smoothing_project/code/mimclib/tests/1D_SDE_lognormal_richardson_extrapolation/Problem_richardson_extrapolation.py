...
#methods
    ...
    def objfun(self,nelem,y):
        if self.smooth==0:
            ...
            #level 1: richardson extrapol (old maybe wrong_way)
            # n=1
            # d = np.array( [[0] * (n + 1)] * (n + 1), float )
            # y10=y[self.N-1]
            # y0_aux=y[0:self.N-1]
            # d[0,0] = self.payoff_smooth2(self.stock_price_trajectory_1D_BS(y10,y0_aux,self.N)) # approximation
            # powerOf4 = 1  # values of 4^j
            # y11=y[2*self.N-1]
            # y1_aux=y[0:2*self.N-1]
            # d[1,0] = self.payoff_smooth2(self.stock_price_trajectory_1D_BS(y11,y1_aux,2*self.N)) # approximation

            # powerOf4 = 4 * powerOf4
            # d[1,1] = d[1,0] + ( d[1,0] - d[0,0] ) / ( powerOf4 - 1 )
            # QoI=d[1,1] 

               #level 1: richardson extrapol (second way)
            ...
            # y1=y[self.N-1]
            # y_aux=y[0:self.N-1]
            # d[0,0] = self.stock_price_trajectory_1D_BS(y1,y_aux,self.N) # approximation
            # y1=y[2*self.N-1]
            # y_aux=y[0:2*self.N-1]
            # d[1,0] = self.stock_price_trajectory_1D_BS(y1,y_aux,2*self.N) # approximation
            # d[1,1] = 2*d[1,0] - d[0,0]
            # QoI=d[1,1] 


             #level 1: richardson extrapol (third way)


            n=1
            d = np.array( [[0] * (n + 1)] * (n + 1), float )

            y_aux_f=y[0:2*self.N]
            y1f=y_aux_f[2*self.N-1]
            y_auxf=y_aux_f[0:2*self.N-1]
            #coarser level points
            y_auxc=y_aux_f[0:self.N-1]
            y1c=y1f
           
           # y_aux_c=[sum(y_aux_f[current: current+2]) for current in xrange(0, len(y_aux_f), 2)]

            d[0,0] = self.stock_price_trajectory_1D_BS(y1c,y_auxc,self.N) # approximation
            
            d[1,0] = self.stock_price_trajectory_1D_BS(y1f,y_auxf,2*self.N) # approximation
            d[1,1] = 2*d[1,0] - d[0,0]
            QoI=d[1,1] 
    ...
    def richardson( self,y,Nsteps, n): 
        ...
        #level 1: richardson extrapol (this was first way, it may be wrong)

        # y1=y[Nsteps-1]
        # y_aux=y[0:Nsteps-1]
        # d[0,0] = self.stock_price_trajectory_1D_BS(y1,y_aux,Nsteps) # approximation
        # powerOf4 = 1  # values of 4^j
        # y1=y[2*Nsteps-1]
        # y_aux=y[0:2*Nsteps-1]
        # d[1,0] = self.stock_price_trajectory_1D_BS(y1,y_aux,2*Nsteps) # approximation
...
# This function implements the brownian bridge construction in 1 D, the argument y here represent independent  multivariate gaussian  rdv  given by
    ...
    def f(self,y1,y,Nsteps):# need to check this
        ...
        dt_s=self.T/float(Nsteps)
        fi=1+(self.sigma/float(np.sqrt(self.T)))*y1*(self.dt_s)+self.sigma*dbb
        product=np.prod(fi)
        summation=np.sum(1/fi)
        Py=product-(self.K/float(self.S0))
        dPy=(self.sigma/float(np.sqrt(self.T)))*(self.dt_s)*product*summation
        return Py,dPy    
