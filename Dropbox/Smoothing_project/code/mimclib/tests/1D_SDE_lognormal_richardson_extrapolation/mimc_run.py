...
class MyRun:
    ...
    def initRun(self, run):
        self.prev_val = 0
        self.prb = Problem_richardson_extrapolation(run.params,1) 
        #self.extrapolate_s_dims = self.prb.N 
        if self.prb.smooth==0:
            self.extrapolate_s_dims = self.prb.N-1 #SET TO SAME AS INPUT DIMENSION
        else:
    ...
    ##
        ...
        if self.prb.smooth==0:
            ...
            self.misc = misc.MISCSampler(d=0, fnKnots=fnKnots, min_dim=2*self.prb.N)
