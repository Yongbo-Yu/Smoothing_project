...
#methods
    ...
    def objfun(self,nelem,y):
        ...
        # to be updated   
        x_l=np.asarray([self.stock_price_trajectory_basket_BS(bar_y1-points_left[i,0],points_left[i,2:self.N+1], points_left[i,1],points_left[i,self.N+1:])[0]  for i in range(0,len(self.yknots_left[0]))])
        ...
        x_r=np.asarray([self.stock_price_trajectory_basket_BS(points_right[i,0]+bar_y1,points_right[i,2:self.N+1], points_right[i,1],points_right[i,self.N+1:])[0] for i in range(0,len(self.yknots_right[0]))])
