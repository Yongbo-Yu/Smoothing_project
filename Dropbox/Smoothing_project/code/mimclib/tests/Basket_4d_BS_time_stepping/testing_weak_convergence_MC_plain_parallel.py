...
#methods
    ...
    def __init__(self,Nsteps,nested=False):
        ...
        self.rho=toeplitz([1,0.3,0.3,0.3]) #correlation matrix
    ...
    def objfun(self,Nsteps): 
        ...
        y4_1=y[3*(Nsteps)+1:self.basket_d*(Nsteps)]
        y41=y[3*(Nsteps)]

        y3_1=y[2*(Nsteps)+1:3*(Nsteps)]
        y31=y[2*(Nsteps)]


        y2_1=y[Nsteps+1:2*(Nsteps)]
        y21=y[Nsteps]
        ...
        #step2
        x=self.stock_price_trajectory_basket_BS(y1,y1_1,y21,y2_1,y31,y3_1,y41,y4_1,Nsteps)  [0] 
    ...
    def stock_price_trajectory_basket_BS(self,y1,yvec_1,y2,yvec_2,y3,yvec_3,y4,yvec_4,Nsteps):
        ...
        bb3=self.brownian_increments(y3,yvec_3,Nsteps)
        bb4=self.brownian_increments(y4,yvec_4,Nsteps)
...
def weak_convergence_differences():    
        ...
        exact= 11.045259  #S_0=K=100, sigma =0.4, corr=0.3, T=1
        marker=['>', 'v', '^', 'o', '*','+','-',':']
        ax = figure().gca()
        ...
        for i in range(0,4):
            ...
            values[:,i]= p.map(processInput, range(((6*(10**7)))))  
        ...
        stand=np.std(values, axis = 0)/  float(np.sqrt(6*(10**7)))
        ...
        print error_diff 
        stand_diff=np.std(differences, axis = 1)/ float(np.sqrt(6*(10**7)))
        print stand_diff
        ...
        plt.savefig('./results/weak_convergence_order_basket_option_4d_1_relative_M_6_10_7_plain.eps', format='eps', dpi=1000)  
        ...
        plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.15, bottom=0.22, right=0.96, top=0.96)
        plt.legend(loc='upper left')
        plt.savefig('./results/weak_convergence_order_differences_basket_option_4d_1_relative_M_6_10_7_plain.eps', format='eps', dpi=1000)  
