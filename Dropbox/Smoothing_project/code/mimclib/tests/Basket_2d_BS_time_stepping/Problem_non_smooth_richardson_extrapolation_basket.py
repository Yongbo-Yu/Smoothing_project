...
#methods
    ...
    def objfun(self,nelem,y):
        ...
        #finer level
        # step 1
        y2_1f=y[2*self.N-1:self.basket_d*(2*self.N-1)]
        #y21f=y[2*self.N]
        ...
        #coarse level
         ...
         # step 1
        y2_1_c=y2_1f[0:(self.N-1)]
        #y21_c=y_aux_c[self.N]
   
        
        y1_1_c=y1_1f[0:self.N-1]
        #y1_c=y_aux_c[0]
        ...
        mylist_right_right_c=mylist_right1_c+mylist_right2_c
        ...
        pay_rl_c=np.multiply(pay_rl_c,(((1/np.sqrt(2 * np.pi))**self.basket_d) * np.exp(-((points_right_left_c[:,0]+bar_z_c[0])**2)/2)\
