...
#methods
    ...
    def objfun(self,Nsteps):
        ...
        #finer level
        # step 1
        y2_1f=y[2*Nsteps+1:self.basket_d*2*Nsteps]
        y21f=y[2*Nsteps]
        ...
        x_f=self.stock_price_trajectory_basket_BS(y1f,y1_1f,y21f,y2_1f,2*Nsteps)  [0]  

        
        d[1,0]=self.payoff(x_f)
        ...
        #coarse level
         ...
         # step 1
        y2_1_c=y2_1f[1:Nsteps]
        y21_c=y21f
   
        
        y1_1_c=y1_1f[1:Nsteps]
        y1_c=y1f
       
        


        x_c=self.stock_price_trajectory_basket_BS(y1_c,y1_1_c,y21_c,y2_1_c,Nsteps)  [0]  

        
        d[0,0]=self.payoff(x_c)
    ...
    def stock_price_trajectory_basket_BS(self,y1,yvec_1,y2,yvec_2,Nsteps):
        ...
        dbb1=dW1-(dt_s/np.sqrt(self.T))*y1 # brownian bridge increments dbb_i (used later for the location of the kink point)
        dbb2=dW2-(dt_s/np.sqrt(self.T))*y2 # brownian bridge increments dbb_i (used later for the location of the kink point)
...
def weak_convergence_differences():   
    ...
    print error_diff 
    stand_diff=np.std(differences, axis = 1)/ float(np.sqrt(4*(10**7)))
    print stand_diff
    ...
    plt.savefig('./results/weak_convergence_order_basket_option_2d_1_richardson_relative_M_4_10_7_plain.eps', format='eps', dpi=1000)  
    ...
    plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.15, bottom=0.22, right=0.96, top=0.96)
    plt.legend(loc='upper left')
    plt.savefig('./results/weak_convergence_order_differences_basket_option_2d_1_richardson_relative_M_4_10_7_plain.eps', format='eps', dpi=1000)  
