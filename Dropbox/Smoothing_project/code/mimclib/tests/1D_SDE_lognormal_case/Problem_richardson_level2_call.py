...
#methods
    ...
    def objfun(self,nelem,y):
        ...
        #finer level points 
        y_aux_ff=y[0:4*self.N-1]
        #kink points by newton method
        bar_z_ff=self.newtons_method(0,y_aux_ff,4*self.N)
        ...
        x_l_ff=np.asarray([self.stock_price_trajectory_1D_BS(bar_z_ff-points_left_ff[i,0],points_left_ff[i,1:],4*self.N)[0]  for i in range(0,len(yknots_left[0]))])
        ...
        x_r_ff=np.asarray([self.stock_price_trajectory_1D_BS(points_right_ff[i,0]+bar_z_ff,points_right_ff[i,1:],4*self.N)[0] for i in range(0,len(yknots_right[0]))])
