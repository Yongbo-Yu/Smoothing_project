...
class MyRun:   
    ...
    def workModel(self, run, lvls):
        ...
        #print(np.prod(beta**(mat*gamma), axis=1))
    ...
    def initRun(self, run):
        ...
        self.extrapolate_s_dims = self.prb.basket_d-1 ; #SET TO SAME AS INPUT DIMENSION
	...
	## PROBLEM SPECIFIC
        ...
        fnKnots= lambda beta: misc.knots_gaussian(misc.lev2knots_doubling(1+beta),   0, 1.0)
        
        # Construct MiscSampler class with the following parameters
        self.misc = misc.MISCSampler(d=0, fnKnots=fnKnots, min_dim= self.prb.basket_d-1 ) # min_dim : minimum dimension between the prob dim and parameters
        
        # those are the error and work contribution used to compute the profit
        self.d_err_rates = 2.*np.log(run.params.beta);#
        #self.d_err_rates=None
