...
def weak_convergence_differences():    
        ...
        for i in range(0,2):
            ...
            def processInput(j):
                ...
                prb.zff=RBergomi.RBergomiST( prb.x,  prb.HIn, prb.e,  prb.r,  prb.T, prb.k,  4*Nsteps_arr[i], prb.MIn)
                prb.dt=prb.T[0]/float(Nsteps_arr[i]) # time steps length
            ...
            results= p.map(processInput, range(3*(10**6)))
            values[:,i]=results    
        ...
        stand=np.std(values, axis = 0)/  float(np.sqrt(3*(10**6)))
        ...
        print error_diff 
        stand_diff=np.std(differences, axis = 1)/ float(np.sqrt(3*(10**6)))
        print stand_diff
        ...
        plt.savefig('./results/weak_convergence_order_Bergomi_H_007_K_1_richardson_level2_relative_M_3_10_6_2.eps', format='eps', dpi=1000)  

        fig = plt.figure()
        plt.plot(dt_arr[0:1], error_diff,linewidth=2.0,label='weak_error' ,marker='>', hold=True) 
        ...
        plt.plot(dt_arr[0:1], fit_diff*10,linewidth=2.0,label=r'rate= %s' % format(z_diff[0]  , '.2f'), linestyle = '--')
        plt.plot(dt_arr[0:1], fit3diff,linewidth=2.0,label=r'rate= %s' % format(z3diff[0]  , '.2f'), linestyle = '--')


        plt.ylabel(r'$\mid  g(X_{\Delta t})-  g(X_{\Delta t/2}) \mid $',fontsize=14) 
        plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.15, bottom=0.22, right=0.96, top=0.96)
        plt.legend(loc='upper left')
        plt.savefig('./results/weak_convergence_order_differences_Bergomi_H_007_K_1_richardson_level2_relative_M_3_10_6_2.eps', format='eps', dpi=1000)
