...
def weak_convergence_differences():    
        ...
        for i in range(0,4):
            ...
            for j in range(10**6):
                ...
                values[j,i]=prb.objfun(Nsteps_arr[i])
    
    

        
        error=np.abs(np.mean(values,axis=0)) 
        stand=np.sqrt(np.std(values, axis = 0)/(10**6))
        Ub=np.abs(np.mean(values,axis=0))+1.96*stand
        Lb=np.abs(np.mean(values,axis=0))-1.96*stand
        print(error)   
        print(stand)
        print Lb
        print Ub
        ...
        plt.ylabel(r'$\mid E [ 3*g(X_{\Delta t/2})-  g(X_{\Delta t})- 2*g(X_{\Delta t/4})]  \mid $',fontsize=14) 
        plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.15, bottom=0.22, right=0.96, top=0.96)
        plt.legend(loc='upper left')
        plt.savefig('./results/weak_convergence_order_differences_higher_depend_Bergomi_H_007_K_1_richardson.eps.eps', format='eps', dpi=1000)  
