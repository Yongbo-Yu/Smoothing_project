...
class Problem_richardson_extrapolation_level2_change_measure(object):
...
#methods
    ...
    def __init__(self,params,nested=False):
        ...
        if self.N==1:
            print 'hello'
            self.z_bar_ff=np.ones((4*self.N))
    ...
    def fun_ff(self,y11):        
        yperp=np.zeros((4*self.N))
        y=np.array([y11,yperp]).reshape(8*self.N)
        #hierarchical
        yperp_1=yperp[1:4*self.N]
        yperp1=yperp[0]
        bbperp=self.brownian_increments(yperp1,yperp_1,4*self.N)
        W1perp= [(bbperp[i+1]-bbperp[i]) *np.sqrt(4*self.N) for i in range(0,len(bbperp)-1)]

        #hierarchical way
        y_1=y11[1:4*self.N]
        y1=y11[0]
        bb=self.brownian_increments(y1,y_1,4*self.N)
        W1= [(bb[i+1]-bb[i]) *np.sqrt(4*self.N) for i in range(0,len(bb)-1)]
        
        QoI=(self.zff.ComputePayoffRT_single(W1,W1perp))*((2*np.pi)**(-4*self.N))*(np.exp(-0.5*y.dot(y)))
        #QoI=(self.z.ComputePayoffRT_single(W1,W1perp))*((2*np.pi)**(-self.N/2))*(np.exp(-0.5*y11.dot(y11)))
        QoI=np.log((QoI))
    ...
    def fun_ff_4(self,y11):        
        ...
        if self.N==2:
            y12=np.zeros((4))
            y_bar_1=np.array([y11,y12]).reshape(4*self.N)
            y=np.array([y_bar_1,yperp]).reshape(8*self.N)
        elif self.N==4:
            y12=np.zeros((4))
            y13=np.zeros((4))
            y14=np.zeros((4))
           
            y_bar_1=np.array([y11,y12,y13,y14]).reshape(4*self.N)
            ...
            y_bar_1=np.array([y11,y12,y13,y14,y15,y16,y17,y18]).reshape(4*self.N)
    ...
    def objfun(self,nelem,y):
        ...
        if self.N==1:
            ...
            y_bar_1_c=bar_y_c
            #
            cst_ff=np.linalg.det(self.Lff)*(2**(2*self.N))*(np.exp(0.5*y[0:4*Nsteps].dot(y[0:4*Nsteps])))
            ...
            y12ff=y[4:8]
            y_bar_1_ff=np.array([bar_y_ff,y12ff]).reshape(4*self.N)

            
            y_bar_1_f=bar_y_f
            y_bar_1_c=bar_y_c
            #
            cst_ff=np.linalg.det(self.Lff)*(2**(self.N))*(np.exp(0.5*y[0:4].dot(y[0:4])))
            cst_f=np.linalg.det(self.Lf)*(2**(self.N))*(np.exp(0.5*y[0:2*Nsteps].dot(y[0:2*Nsteps])))
            cst_c=np.linalg.det(self.Lc)*(2**(self.N/2))*(np.exp(0.5*y[0:Nsteps].dot(y[0:Nsteps])))
            ...
            if self.N==4:
                ...
                y14ff=y[12:16]
                #y12c=y[2:4]
                y_bar_1_ff=np.array([bar_y_ff,y12ff,y13ff,y14ff]).reshape(4*self.N)
                ...
                cst_ff=np.linalg.det(self.Lff)*(2**(self.N/2))*(np.exp(0.5*y[0:4].dot(y[0:4])))
                ...
                y_bar_1_ff=np.array([bar_y_ff,y12ff,y13ff,y14ff,y15ff,y16ff,y17ff,y18ff]).reshape(4*self.N)
                y_bar_1_f=np.array([bar_y_f,y12_f,y13_f,y14_f]).reshape(2*self.N)
                y_bar_1_c=np.array([bar_y_c,y12_c]).reshape(self.N)



                cst_ff=np.linalg.det(self.Lff)*(2**(self.N/4))*(np.exp(0.5*y[0:4].dot(y[0:4])))
