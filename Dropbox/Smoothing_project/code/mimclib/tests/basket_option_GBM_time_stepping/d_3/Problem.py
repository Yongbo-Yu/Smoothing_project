...
# attributes
    ...
    exact= 11.70 # 3-d, sigma=0.4, S_0=K=100, T=1, r=0,rho=0.3
...
#methods
    ...
    def objfun(self,nelem,y):
        ...
        x_l=np.asarray([self.stock_price_trajectory_basket_BS(bar_y1-points_left[i,0],points_left[i,3:self.N+2]\
                                                            ,points_left[i,1],points_left[i,self.N+2:2*self.N+1]\
                                                            ,points_left[i,2],points_left[i,2*self.N+1:] )[0]  for i in range(0,len(yknots_left[0]))])
        ...
        # to be updated    (we start with the case d=2)  
        x_r=np.asarray([self.stock_price_trajectory_basket_BS(points_right[i,0]+bar_y1,points_right[i,3:self.N+2]\
                                                              ,points_right[i,1],points_right[i,self.N+2:2*self.N+1]\
                                                              ,points_right[i,2],points_right[i,2*self.N+1:])[0] for i in range(0,len(yknots_right[0]))])
    ...
    def f(self,y1,y__1,z__1):# need to check this for case d=2, N=2 and then we can extend
        ...
        Py=(self.S0[0]*self.c[0]*product[0]+ self.S0[1]*self.c[1]*product[1]+ self.S0[2]*self.c[2]*product[2])-self.K    
