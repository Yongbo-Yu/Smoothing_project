...
# attributes
   ...
   # y1perp = Vector(N)
    ...
    def __init__(self,Nsteps,nested=False):
        ...
        self.L=self.rfbm.GetL()  #getting the L matrix
        L11=L[0:Nsteps]
    ...
    def objfun(self,Nsteps):
        ...
        X[0:Nsteps]=(np.linalg.inv(self.L1)).dot(W1)   #constructing X[0:Nsteps-1]

        X[Nsteps:]=y[Nsteps:]   #constructing X[Nsteps:]

    
        # Step 3:  given x compute W1 and Wtilde
        rfbm.generate(X,W1, Wtilde); # need to adjust rfbm code
        # Step 4:  compute price
        QoI=updatePayoff_cholesky(Wtilde,W1,v,eta,H,rho,xi,t,K,N) 
        return QoI
...
def weak_convergence_differences():    
        ...
        for i in range(0,4):
            ...
            values[:,i]= p.map(processInput, range(((3*(10**6)))))    
        ...
        plt.savefig('./results/weak_convergence_order_Bergomi_H_002_K_12_M_3_10_6_CI_relative.eps', format='eps', dpi=1000)  
        ...
        plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.15, bottom=0.22, right=0.96, top=0.96)
        plt.legend(loc='upper left')
        plt.savefig('./results/weak_convergence_order_differences_Bergomi_H_002_K_12_M_3_10_6_CI_relative.eps', format='eps', dpi=1000)  
