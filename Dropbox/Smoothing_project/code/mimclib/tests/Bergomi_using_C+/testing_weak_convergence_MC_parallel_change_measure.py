   ...
   # y1perp = Vector(N)
    ...
    def fun_4(self,y11):        
        ...
        if Nsteps==8:
            ...
            y_bar_1=np.array([y11,y12,y13,y14]).reshape(Nsteps)
    ...
    def objfun(self,Nsteps):
        ...
        if Nsteps<=4:
            y_bar_1=np.sqrt(2)*np.dot(self.L,y[0:Nsteps])+self.z_bar1
            bar_y=y_bar_1
            cst=np.linalg.det(self.L)*(2**(Nsteps/2))*(np.exp(0.5*y[0:Nsteps].dot(y[0:Nsteps])))
        else:
            bar_y=np.sqrt(2)*np.dot(self.L,y[0:4])+self.z_bar1
            if Nsteps==8:
                #hierarchical way
                y12=y[4:8]
                y_bar_1=np.array([bar_y,y12]).reshape(Nsteps)
                cst=np.linalg.det(self.L)*(2**(Nsteps/4))*(np.exp(0.5*y[0:4].dot(y[0:4])))
            else:
                #hierarchical way
                y12=y[4:8]
                y13=y[8:12]
                y14=y[12:16]
                y_bar_1=np.array([bar_y,y12,y13,y14]).reshape(Nsteps)
                cst=np.linalg.det(self.L)*(2**(Nsteps/8))*(np.exp(0.5*y[0:4].dot(y[0:4])))
...
def weak_convergence_differences():    
        ...
        plt.savefig('./results/weak_convergence_order_Bergomi_H_007_K_1_M_10_5_CI_relative_measure_change.eps', format='eps', dpi=1000)  
        ...
        plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.15, bottom=0.22, right=0.96, top=0.96)
        plt.legend(loc='upper left')
        plt.savefig('./results/weak_convergence_order_differences_Bergomi_H_007_K_1_M_10_5_CI_relative_measure_change.eps', format='eps', dpi=1000)  
