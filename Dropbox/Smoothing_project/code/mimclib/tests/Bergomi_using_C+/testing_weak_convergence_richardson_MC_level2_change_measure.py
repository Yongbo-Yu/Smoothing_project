...
# attributes
   ...
   # y1perp = Vector(N)
    ...
    def __init__(self,Nsteps,nested=False):
        ...
        if  Nsteps==1:
            self.N=Nsteps
            self.z_bar_ff=np.ones((4* Nsteps))
            self.z_bar_f=np.ones((2* Nsteps))
            self.z_bar_c=np.ones(( Nsteps))
            
            from scipy.optimize import minimize    

            mini_ff= minimize(self.fun2_ff, self.z_bar_ff, method='Nelder-Mead', options={'xatol': 1e-5, 'disp': False})
            self.z_bar_ff_1=mini_ff.x
            

            self.Hfun_ff = nd.Hessian(self.fun_ff) 
            Hfun_mode_ff=np.linalg.inv(-self.Hfun_ff(self.z_bar_ff_1))
             # spectral
            e_vals_ff, e_vecs_ff = np.linalg.eig(Hfun_mode_ff)
            self.Lff=e_vecs_ff.dot(np.diag(np.sqrt(e_vals_ff)))

            #Cholesky
            #self.Lff=np.linalg.cholesky(Hfun_mode_ff)
            #print self.Lff
            ...
            mini_ff= minimize(self.fun2_ff_4, self.z_bar_ff, method='Nelder-Mead', options={'xatol': 1e-5, 'disp': False})
            self.z_bar_ff_1=mini_ff.x
            

            self.Hfun_ff = nd.Hessian(self.fun_ff_4) 
            Hfun_mode_ff=np.linalg.inv(-self.Hfun_ff(self.z_bar_ff_1))
             # spectral
            e_vals_ff, e_vecs_ff = np.linalg.eig(Hfun_mode_ff)
            self.Lff=e_vecs_ff.dot(np.diag(np.sqrt(e_vals_ff)))
    ...
    def fun_ff(self,y11):     
        ...
        QoI=(self.zff.ComputePayoffRT_single(W1,W1perp))*((2*np.pi)**(-Nsteps))*(np.exp(-0.5*y.dot(y)))
        #QoI=(self.z.ComputePayoffRT_single(W1,W1perp))*((2*np.pi)**(-self.N/2))*(np.exp(-0.5*y11.dot(y11)))
        QoI=np.log((QoI))
    ...
    def fun2_c_4(self,y11):  
        ...
        QoI=(self.z.ComputePayoffRT_single(W1,W1perp))*((2*np.pi)**(- Nsteps))*(np.exp(-0.5*y.dot(y)))
        #QoI=(self.z.ComputePayoffRT_single(W1,W1perp))*((2*np.pi)**(- Nsteps/2))*(np.exp(-0.5*y11.dot(y11)))
        QoI=-np.log((QoI))
    ...
    def fun_ff_4(self,y11):        
        Nsteps=self.N
        yperp=np.zeros((4*Nsteps))

        if Nsteps==2:
            y12=np.zeros((4))
            y_bar_1=np.array([y11,y12]).reshape(4*Nsteps)
            y=np.array([y_bar_1,yperp]).reshape(8*Nsteps)
        elif Nsteps==4:
            y12=np.zeros((4))
            y13=np.zeros((4))
            y14=np.zeros((4))
           
            y_bar_1=np.array([y11,y12,y13,y14]).reshape(4*Nsteps)
            ...
            y_bar_1=np.array([y11,y12,y13,y14,y15,y16,y17,y18]).reshape(4*Nsteps)

            y=np.array([y_bar_1,yperp]).reshape(8*Nsteps) 

        
        #hierarchical
        yperp_1=yperp[1:4*Nsteps]
        yperp1=yperp[0]
        bbperp=self.brownian_increments(yperp1,yperp_1,4*Nsteps)
        W1perp= [(bbperp[i+1]-bbperp[i]) *np.sqrt(4*Nsteps) for i in range(0,len(bbperp)-1)]

        #hierarchical way
        y_1=y_bar_1[1:4*Nsteps]
        y1=y_bar_1[0]
        bb=self.brownian_increments(y1,y_1,4*Nsteps)
        W1= [(bb[i+1]-bb[i]) *np.sqrt(4*Nsteps) for i in range(0,len(bb)-1)]
        
        QoI=(self.zff.ComputePayoffRT_single(W1,W1perp))*((2*np.pi)**(-4*Nsteps))*(np.exp(-0.5*y.dot(y)))
        #QoI=(self.z.ComputePayoffRT_single(W1,W1perp))*((2*np.pi)**(-self.N/2))*(np.exp(-0.5*y11.dot(y11)))
        QoI=np.log((QoI))
    ...
    def fun2_ff_4(self,y11):        
        ...
        yperp1=yperp[0]
        bbperp=self.brownian_increments(yperp1,yperp_1,4*Nsteps)
        W1perp= [(bbperp[i+1]-bbperp[i]) *np.sqrt(4*self.N) for i in range(0,len(bbperp)-1)]
    ...
    def objfun(self,Nsteps):
        ...
        if Nsteps==1:
            bar_y_ff=np.sqrt(2)*np.dot(self.Lff,y[0:4*Nsteps])+self.z_bar_ff_1
            ...
            y_bar_1_c=bar_y_c
            #
            cst_ff=np.linalg.det(self.Lff)*(2**(2*Nsteps))*(np.exp(0.5*y[0:4*Nsteps].dot(y[0:4*Nsteps])))
            ...
            bar_y_ff=np.sqrt(2)*np.dot(self.Lff,y[0:4])+self.z_bar_ff_1
            bar_y_f=np.sqrt(2)*np.dot(self.Lf,y[0:2*Nsteps])+self.z_bar_f_1
            bar_y_c=np.sqrt(2)*np.dot(self.Lc,y[0:Nsteps])+self.z_bar_c_1

            y12ff=y[4:8]
            y_bar_1_ff=np.array([bar_y_ff,y12ff]).reshape(4*Nsteps)

            
            y_bar_1_f=bar_y_f
            y_bar_1_c=bar_y_c
            #
            cst_ff=np.linalg.det(self.Lff)*(2**(Nsteps))*(np.exp(0.5*y[0:4].dot(y[0:4])))
            cst_f=np.linalg.det(self.Lf)*(2**(Nsteps))*(np.exp(0.5*y[0:2*Nsteps].dot(y[0:2*Nsteps])))
            cst_c=np.linalg.det(self.Lc)*(2**(Nsteps/2))*(np.exp(0.5*y[0:Nsteps].dot(y[0:Nsteps])))
            ...
            if Nsteps==4:
                ...
                y14ff=y[12:16]
                #y12c=y[2:4]
                y_bar_1_ff=np.array([bar_y_ff,y12ff,y13ff,y14ff]).reshape(4*Nsteps)
                ...
                cst_ff=np.linalg.det(self.Lff)*(2**(Nsteps/2))*(np.exp(0.5*y[0:4].dot(y[0:4])))
                cst_f=np.linalg.det(self.Lf)*(2**(Nsteps/2))*(np.exp(0.5*y[0:4].dot(y[0:4])))
                cst_c=np.linalg.det(self.Lc)*(2**(Nsteps/2))*(np.exp(0.5*y[0:4].dot(y[0:4])))
                ...
                y_bar_1_ff=np.array([bar_y_ff,y12ff,y13ff,y14ff,y15ff,y16ff,y17ff,y18ff]).reshape(4*Nsteps)
                y_bar_1_f=np.array([bar_y_f,y12_f,y13_f,y14_f]).reshape(2*Nsteps)
                y_bar_1_c=np.array([bar_y_c,y12_c]).reshape(Nsteps)



                cst_ff=np.linalg.det(self.Lff)*(2**(Nsteps/4))*(np.exp(0.5*y[0:4].dot(y[0:4])))
        ...
        dc = self.z.ComputePayoffRT_single(W1_c,W1perp_c)*(np.exp(-0.5*bar_y_c.dot(bar_y_c)))*cst_c; 
        df=self.zf.ComputePayoffRT_single(W1_f,W1perp_f)*(np.exp(-0.5*bar_y_f.dot(bar_y_f)))*cst_f;  
        dff=self.zff.ComputePayoffRT_single(W1_ff,W1perp_ff)*(np.exp(-0.5*bar_y_ff.dot(bar_y_ff)))*cst_ff;  
...
def weak_convergence_differences():    
        ...
        for i in range(0,4):
            ...
            p =  pp.ProcessPool(num_cores)  # Processing Pool with num_cores processors
        ...
        plt.savefig('./results/weak_convergence_order_Bergomi_H_007_K_1_richardson_level2_relative_change_measure_M_10_5_spec.eps', format='eps', dpi=1000)  
        ...
        plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.15, bottom=0.22, right=0.96, top=0.96)
        plt.legend(loc='upper left')
        plt.savefig('./results/weak_convergence_order_differences_Bergomi_H_007_K_1_richardson_level2_relative_change_measure_M_10_5_spec.eps', format='eps', dpi=1000)
