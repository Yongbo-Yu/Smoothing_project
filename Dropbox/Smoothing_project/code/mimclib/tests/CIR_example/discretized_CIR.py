...
#Constructing the Brownian brdidge increments
...
def stock_price_trajectory_1D_CIR_full_truncation(T,d,X0,sigma,a,b):
    ...
    dbb=dW-(dt/np.sqrt(T))*( bb[-1]/np.sqrt(T))
    ...
    plt.title('1D CIR process  ($X_0$=%s,T=%s,$\sigma$=%s,$a$=%s,$b$=%s)'%(X0,T,sigma,a,b),fontsize=14)  
    plt.savefig('./results/1D_CIR_FE_4.eps', format='eps', dpi=1000)    
    return X
