...
def weak_convergence_differences():    
        ...
        exact=  6.437949#S_0=K=100, sigma =0.2, corr=0.3, T=1
        marker=['>', 'v', '^', 'o', '*','+','-',':']
        ax = figure().gca()
        ...
        print error_diff 
        stand_diff=np.std(differences, axis = 1)/ float(np.sqrt(1*(10**7)))
        print stand_diff
        ...
        plt.savefig('./results/weak_convergence_order_basket_option_2d_1_relative_M_1_10_7_plain.eps', format='eps', dpi=1000)  
        ...
        plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.15, bottom=0.22, right=0.96, top=0.96)
        plt.legend(loc='upper left')
        plt.savefig('./results/weak_convergence_order_differences_basket_option_2d_1_relative_M_1_10_7_plain.eps', format='eps', dpi=1000)  
