...
parser.add_argument("-db_tag", type=str, action="store",
                    default="1DBS_4_steps_non_smooth_richardson_without_middle_only_coarser", help="Database tag")
...
#case of 1 D problem
if dim == 0:
  base = "mimc_run.py -mimc_TOL {TOL} -mimc_min_dim 0  -qoi_dim {the_dim} -ksp_rtol 1e-25 -ksp_type gmres -mimc_M0 1 \
...
if not args.db:
    cmd_single = "python " + base + " -mimc_verbose 10 -db True -db_tag 1DBS_2_4steps_smooth_payoff2_richardson" #verbose >0 detailed printing results
    ...
    cmd_multi = "python " + base + " -mimc_verbose 0 -db True -db_tag 1DBS_4_steps_non_smooth_richardson_without_middle_only_coarser"
    for i in range(0, args.tries):
       print cmd_multi.format(tag=args.db_tag.format(dim), TOL=0.00001)