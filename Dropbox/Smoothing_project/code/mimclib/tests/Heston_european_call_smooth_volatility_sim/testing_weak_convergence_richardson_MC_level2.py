...
# attributes
    ...
    def objfun(self,Nsteps):
        mean = np.zeros(8*Nsteps-1)
        covariance= np.identity(8*Nsteps-1)
        ...
        y1ff=y[0:4*Nsteps] # this points are related to the volatility path
        ...
        bar_z_ff=self.newtons_method(y2ff[0],y2sff,y1ff[0],y1ff[1:],4*Nsteps)
        ...
        x_l_ff=np.asarray([self.stock_price_trajectory_1D_heston(bar_z_ff-points_left_ff[i,0],points_left_ff[i,1:],y1ff[0],y1ff[1:],4*Nsteps)[0]  for i in range(0,len(self.yknots_left[0]))])
        ...
        x_r_ff=np.asarray([self.stock_price_trajectory_1D_heston(points_right_ff[i,0]+bar_z_ff,points_right_ff[i,1:],y1ff[0],y1ff[1:],4*Nsteps)[0] for i in range(0,len(self.yknots_right[0]))])
        ...
        #finer level QoI
        #finer level points 
        y1f=y1ff[0:2*Nsteps] # this points are related to the volatility path
