...
def first_difference_rate_plotting():       
    ...
    for d1 in range(0,11,1):
        ...
        mylist[:2*prb.N-1]=[fixed_points for i in range(0,3*prb.N-1)] #second way
        j=0
       ...
       # QoI_beta=np.zeros(prb.N,dtype=int)
        QoI_beta=np.zeros(3*prb.N-1,dtype=int) #second way
        QoI_beta[d1]=1
...
def mixed_difference_order2_rate_plotting(d):       
    ...
    for k in range(0,11,1):  
        if k==d:
            ...
            mylist[:3*prb.N-1]=[fixed_points for i in range(0,3*prb.N-1)]# second way
            mylist_weight[:3*prb.N-1]=[fixed_weight for i in range(0,3*prb.N-1)] #second way
            j=0
