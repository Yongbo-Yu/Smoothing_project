...
class MyRun:
    ...
    def initRun(self, run):
        self.prev_val = 0
        self.prb = Problem_general_n_int(1,run.params) 
        #self.prb.BeginRuns()
        #self.extrapolate_s_dims = self.prb.N 
        self.extrapolate_s_dims = 3*self.prb.N-1#SET TO SAME AS INPUT DIMENSION
    ...
    ##
        ...
        self.misc = misc.MISCSampler(d=0, fnKnots=fnKnots, min_dim=3*self.prb.N-1)
...
if __name__ == "__main__":
    from Problem_general_n_int import Problem_general_n_int

    Problem_general_n_int.Init()
    import mimclib.test
    run = MyRun()
