...
#methods
    ...
    def objfun(self,nelem,y):
        ...
        mean = np.zeros(8*self.N-1)
        covariance= np.identity(8*self.N-1)
        ...
        y1ff=y[0:4*self.N] # this points are related to the volatility path
        y2ff=[4*self.N]
        y2ff[0]=0.0
        y2ff[1:]=y[4*self.N:]
        ...
        bar_z_ff=self.newtons_method(y2ff[0],y2sff,y1ff[0],y1ff[1:],4*self.N)

        
        # step 3: performing the pre-intgeration step wrt kink point
    
        mylist_left_ff=[]
        mylist_left_ff.append(self.yknots_left[0])
        mylist_left_ff[1:]=[np.array(y2sff[i]) for i in range(0,len(y2sff))]
        points_left_ff=self.cartesian(mylist_left_ff)

        x_l_ff=np.asarray([self.stock_price_trajectory_1D_heston(bar_z_ff-points_left_ff[i,0],points_left_ff[i,1:],y1ff[0],y1ff[1:],4*self.N)[0]  for i in range(0,len(self.yknots_left[0]))])
        QoI_left_ff= self.yknots_left[1].dot(self.payoff(x_l_ff)*((1/np.sqrt(2 * np.pi)) * np.exp(-((bar_z_ff-points_left_ff[:,0])**2)/2)* np.exp(points_left_ff[:,0])))


        mylist_right_ff=[]
        mylist_right_ff.append(self.yknots_right[0])
        mylist_right_ff[1:]=[np.array(y2sff[i]) for i in range(0,len(y2sff))]
        points_right_ff=self.cartesian(mylist_right_ff)
        x_r_ff=np.asarray([self.stock_price_trajectory_1D_heston(points_right_ff[i,0]+bar_z_ff,points_right_ff[i,1:],y1ff[0],y1ff[1:],4*self.N)[0] for i in range(0,len(self.yknots_right[0]))])
        QoI_right_ff= self.yknots_right[1].dot(self.payoff(x_r_ff)*(1/np.sqrt(2 * np.pi)) * np.exp(-((points_right_ff[:,0]+bar_z_ff)**2)/2)* np.exp(points_right_ff[:,0]))
        ...
        #finer level QoI
        #finer level points 
        y1f=y1ff[0:2*self.N] # this points are related to the volatility path
    ...
    def f(self,y1,y,yv1,yv,Nsteps):
        X,dbb,V,y1s=self.stock_price_trajectory_1D_heston(y1,y,yv1,yv,Nsteps) # right version
