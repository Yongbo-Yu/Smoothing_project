...
#methods
    ...
    def objfun(self,nelem,y):
        ...
        y2=y[self.N+1:2*(self.N+1)]

        y_s= self.rho *y1+ np.sqrt(1-self.rho**2) * y2
      
        
        ys=y_s[1:self.N+1]
     
        
        bar_z=self.newtons_method(0,ys,y1[0],y1[1:self.N+1])
        ...
        x_l=np.asarray([self.stock_price_trajectory_1D_BS(bar_z-points_left[i,0],points_left[i,1:],y1[0],y1[1:self.N+1])[0]  for i in range(0,len(yknots_left[0]))])
        ...
        x_r=np.asarray([self.stock_price_trajectory_1D_BS(points_right[i,0]+bar_z,points_right[i,1:],y1[0],y1[1:self.N+1])[0] for i in range(0,len(yknots_right[0]))])
    ...
    def stock_price_trajectory_1D_BS(self,y1,y,yv1,yv):
        ...
        V=np.zeros(self.N+2) #here will store the  CIR trajectory
        X[0]=self.S0
        V[0]=self.v0
        for n in range(1,self.N+2):
            V[n]=V[n-1]*(1-self.a *self.dt)+ self.sigma *np.sqrt(max(V[n-1],0))*dW_v[n-1]+self.a*self.b*self.dt
            X[n]=X[n-1]*(1+V[n-1]*dW[n-1])
    ...
    def f(self,y1,y,yv1,yv):# need to check this
        X,dbb=self.stock_price_trajectory_1D_BS(y1,y,yv1,yv) # right version
