...
class Problem_richardson_level2(object):
...
def weak_convergence_differences():    
        ...
        for i in range(0,4):
            ...
            prb = Problem_richardson_level2(1,Nsteps_arr[i]) 
        ...
        stand=np.std(values, axis = 0)/  float(np.sqrt(5*(10**6)))
        ...
        print error_diff 
        stand_diff=np.std(differences, axis = 1)/ float(np.sqrt(5*(10**6)))
        print stand_diff
        ...
        plt.savefig('./results/weak_convergence_order_binary_richardson_level2_relative.eps', format='eps', dpi=1000)  
        ...
        plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.15, bottom=0.22, right=0.96, top=0.96)
        plt.legend(loc='upper left')
        plt.savefig('./results/weak_convergence_order_differences_binary_richardson_level2_relative.eps', format='eps', dpi=1000)
