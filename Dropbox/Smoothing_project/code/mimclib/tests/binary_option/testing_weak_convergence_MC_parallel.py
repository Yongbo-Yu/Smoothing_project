...
def weak_convergence_differences():    
        ...
        for i in range(0,4):
            ...
            values[:,i]= p.map(processInput, range(((10**3))))  
        ...
        stand=np.std(values, axis = 0)/  float(np.sqrt(10**3))
        ...
        print error_diff 
        stand_diff=np.std(differences, axis = 1)/ float(np.sqrt(10**3))
        print stand_diff
        ...
        plt.savefig('./results/weak_convergence_order_binary_option_relative_M_10_3.eps', format='eps', dpi=1000)  
        ...
        plt.subplots_adjust(wspace=0.6, hspace=0.6, left=0.15, bottom=0.22, right=0.96, top=0.96)
        plt.legend(loc='upper left')
        plt.savefig('./results/weak_convergence_order_differences_binary_option_relative_M_10_3.eps', format='eps', dpi=1000)  
