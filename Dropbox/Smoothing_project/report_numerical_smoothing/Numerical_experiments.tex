...
\subsubsection{Discretization of Heston model with a non smooth transformations for the volatility process}\label{sec:Discretization of Heston model with a non smooth transformations for the volatility process}
Given that the SDE for the asset path is now dependent  upon the solution of the second volatility SDE in \eqref{eq:dynamics Heston}, it is necessary to simulate the volatility process first and then utilize this "volatility path" in order to simulate the asset path. In the case of the original GBM SDE it is possible to use It\^o's Lemma to directly solve for $S_t$. However, we are unable to utilize that procedure here and must use a numerical approximation in order to obtain both paths. Furthermore,  to avoid  problems with negative values of the volatility process $v_t$, many non smooth transformations  were introduced in the literature (see \cite{lord2010comparison}). We introduce $f_1, f_2$, and $f_3$, which with different choices implies different schemes, and   apply Forward Euler scheme to descretize \ref{eq:dynamics Heston}  results in
...
\cite{lord2010comparison} suggest that the Full Truncation method is the "best" option in terms of weak error convergence. Therefore,  we use this scheme for this approach. 
...
Since we are using ASGQ which is very sensitive to the smoothness of the integrand. We found numerically (see section \ref{sec:Comparison between the two approaches of simulating the volatlity process}) that using a non smooth transformation to ensure the positiveness of the volatility process deteriorates the performance of the ASGQ. An alternative way to guarantee the positiveness of the volatility process is to simulate it as  the sum of  Ornstein-Uhlenbeck (Bessel) processes.
...
\subsubsection{Comparison between the two approaches of simulating the volatility process}\label{sec:Comparison between the two approaches of simulating the volatlity process}
In this section, we try to compare  the two approaches, discussed  in Sections \ref{sec:Discretization of Heston model with a non smooth transformations for the volatility process} and \ref{sec:Discretization of Heston model with the volatility process Simulated using the sum of  Ornstein-Uhlenbeck (Bessel) processes}, to simulate the volatility process. The comparison is done in terms of mixed differences convergence rates which, as pointed out in \cite{haji2016multi}, are a key determinant for the optimal performance of ASGQ.

For comparison, we use an example of the single European call option under Heston model whose parameters result in $n=1$ (using a  single OU process for simulating the volatility process in approach given by Section \ref{sec:Discretization of Heston model with the volatility process Simulated using the sum of  Ornstein-Uhlenbeck (Bessel) processes}). For instance we tested the example with  that  parameters: $S_0=K=100$, $v_0=0.04$, $\mu=0$,  $\rho=-0.9$, $\kappa=1$, $\xi=0.1$, $\theta=\frac{\xi^2}{4 \kappa}=0.0025$. The reference value in this case is $6.3325$ (computed using Premia with cf\_call\_heston method as explained in \cite{heston1993closed}). For solving the root finding problem we follow similar approach to the one in Section \ref{sec:Determining the kink location} with substituting $\sigma$ with $\sqrt{v_i}$ in $f_i$.
...
\begin{figure}[h!]
	...
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/Heston_single_call_full_truncation_vol/mixed_rates/set2/N_2/first_difference_heston_2steps_hierarchical_2}
		\caption{}
		\label{fig:sub3}
	\end{subfigure}%
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/Heston_single_call_smooth_vol/mixed_rates/set2/N_2/first_difference_heston_2steps_hierarchical_2}
		\caption{}
		\label{fig:sub4}
	\end{subfigure}
	...
	\label{fig:first_diff_Heston_call_N_2}
\end{figure}
\FloatBarrier
...
\begin{figure}[h!]
	...
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/Heston_single_call_full_truncation_vol/mixed_rates/set2/N_4/first_difference_heston_4steps_hierarchical_2}
		\caption{}
		\label{fig:sub3}
	\end{subfigure}%
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/Heston_single_call_smooth_vol/mixed_rates/set2/N_4/first_difference_heston_4steps_hierarchical_2}
		\caption{}
		\label{fig:sub4}
	\end{subfigure}
...
\FloatBarrier
	\begin{figure}[h!]
\centering
\includegraphics[width=0.4\linewidth]{./figures/Heston_single_call_smooth_vol/complexity_rates/error_vs_time}

\caption{Computational work comparison for ASGQ and MC methods, for the case of European call option price under discretized Heston model. This plot shows that to achieve a relative error around $1.5\%$, ASGQ outperforms  MC method in terms of computational time.}
