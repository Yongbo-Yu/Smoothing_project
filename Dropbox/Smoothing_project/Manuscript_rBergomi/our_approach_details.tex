...
\red{On the other hand, the total error of approximating the  expectation in \eqref{BS_formula_rbergomi} using the randomized QMC or MC estimator, $Q^{\text{MC(QMC)}}_N$ can be bounded by
...
\end{align}
 ...
 \end{equation*}
\red{the optimal index set  $\mathcal{I}$  for our ASGQ is given by 
 ...
 \begin{figure}[htb]
 	...
 	\begin{subfigure}{0.165\textwidth}
 		...
 		\label{fig:6}
 	\end{subfigure}
 	\caption{\red{Construction of the index set for ASGQ method. A posteriori, adaptive construction: Given an index set $\mathcal{I}_k$, compute the profits of the neighbor indices and select the most profitable one.}}
