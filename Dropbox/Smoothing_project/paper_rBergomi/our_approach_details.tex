...
\begin{figure}[h!]
	...
	\caption{The rate of convergence of  first order differences $\abs{\Delta \text{E}_{\beta}}$ ($\beta=\mathbf{1}+k \bar{\beta}$) with respect to $W^1$ (a)  and  with respect to $W^2$ (b), for parameter set $3$ in table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}. The number of quadrature points used in the $i$-th dimension is $N_i=2^{\beta_i-1}+1$. }
