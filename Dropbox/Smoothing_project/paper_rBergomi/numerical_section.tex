...
\begin{table}[!h]
	...
	\end{tabular}
\end{small}
	\caption{Reference solution, which is the  approximation of the call option price under the rBergomi model, defined in \eqref{BS_formula_rbergomi},  using MC with $500$ time steps and number of samples, $M=4 \times 10^6$, for different parameter constellations.  The numbers between parentheses correspond to the statistical errors estimates.}
...
In this section, we conduct a comparison between \red{MC, QMC and ASGQ} in terms of errors and computational time. We show tables and plots reporting  the different relative errors involved in the \red{MC and QMC}  methods (bias and statistical error  estimates), and in ASGQ (bias and quadrature error estimates).  While fixing  a  sufficiently small error tolerance in the price estimates,  we also compare the computational time needed for both methods to meet the desired error tolerance.  We note that  in all cases the actual work (runtime) is obtained using an Intel(R) Xeon(R) CPU E$5$-$268$ architecture. 
...
\item[iii)] \red{In order to compare the different methods, the number of samples, $M^{\text{QMC}}$ and $M^{\text{MC}}$, are chosen so that  the statistical errors of QMC, $\mathcal{E}_{S,\text{QMC}}(M^{\text{QMC}})$, and MC, $\mathcal{E}_{S,\text{MC}}(M^{\text{MC}})$, satisfy
...
\begin{table}[!h]
	...
	\end{tabular}
\end{small}
	\caption{Summary of relative errors and computational gains, achieved by the different methods. In this table, \red{we highlight the computational gains achieved by ASGQ and QMC over MC method to meet a certain error tolerance. We note that the ratios are computed for the best configuration with Richardson extrapolation for each method}. We provide details about the way we compute these gains for each case in the following sections.}
...
\begin{table}[h!]
	...
	\hline
		...
		ASGQ ($\text{TOL}_{\text{ASGQ}}=5.10^{-2}$)  & $\underset{(0.96,0.37)}{\mathbf{  1.33}}$ & $\underset{(0.07,0.16)}{\mathbf{ 0.23}}$ & $\underset{(0.015,0.010)}{\mathbf{  0.025}}$   \\
...
%		M(\# MC samples) & $3 \times 10^4$  & $2 \times 10^5$ & $4 \times 10^4$  \\
		...
		QMC &$\underset{(0.96,0.91)}{\mathbf{1.87}}$  & $\underset{(0.07,0.09)}{\mathbf{0.16}}$ & $\underset{(0.015,0.018)}{\mathbf{0.033}}$  \\
		M(\# QMC samples) & $2^3 \times 2^4= 128$  & $2^3 \times 2^{11}= 8192$ & $2^3 \times 2^{14}= 
		131072$ \\
				\hline
		
		MC &$\underset{(0.96,0.92)}{\mathbf{1.88}}$  & $\underset{(0.07,0.07)}{\mathbf{0.14}}$ & $\underset{(0.015,0.015)}{\mathbf{0.03}}$  \\
		M(\# MC samples) & $4 \times 10$  & $8 \times 10^3$ & $16 \times 10^4$  \\

		\bottomrule[1.25pt]
	\end{tabular}
	\caption{Total relative error of the different methods, coupled with Richardson extrapolation (level $1$), to compute the call option price  for different numbers of time steps. The values between parentheses correspond to the different errors contributing to the total relative error: for ASGQ we report the bias and quadrature errors and for MC and QMC we report the bias and the statistical errors. The number of MC and QMC samples, $M$, are chosen to satisfy \eqref{optimal_number_samples}.}
...
%		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-1}$)  & $0.1$ & $0.18$ & $0.3$   \\
		...
		ASGQ ($\text{TOL}_{\text{ASGQ}}=5.10^{-2}$)  & $0.1$ & $0.6$ & $37$  \\
...
%		$  \\
		\bottomrule[1.25pt]
		\end{tabular}
		\caption{Comparison of the computational time (in seconds) of  the different methods coupled with Richardson extrapolation (level $1$), to compute the call option price of the rBergomi model for different numbers of time steps. The average MC CPU time is computed over $100$ runs.}
...
%			M(\# MC samples) & $5 \times 10^4$  & $7 \times 10^5$   \\
				...
				\hline
		QMC   & $\underset{(0.24,0.32)}{\mathbf{0.56}}$  & $\underset{(0.006,0.008)}{\mathbf{0.014}}$    \\
			M(\# QMC samples) & $2^3 \times 2^8=2048$  & $2^3 \times 2^{17}=1048576$   \\			
			
				\hline
		MC   & $\underset{(0.24,0.21)}{\mathbf{0.45}}$  & $\underset{(0.006,0.006)}{\mathbf{0.012}}$    \\
			M(\# MC samples) & $16 \times 10^2$  & $16 \times 10^5$   \\
		\bottomrule[1.25pt]
	\end{tabular}
	\caption{Total relative error of the different methods, coupled with Richardson extrapolation (level $2$), to compute the call option price for different numbers of time steps.  The values between parentheses correspond to the different errors contributing to the total relative error: for ASGQ we report the bias and quadrature errors and for MC and QMC we report the bias and the statistical errors. The number of MC and QMC samples, $M$, are chosen to satisfy \eqref{optimal_number_samples}.}
...
\begin{table}[!h]
	...
	\hline
		...
		ASGQ ($\text{TOL}_{\text{ASGQ}}=5.10^{-2}$)  & $0.5$ & $74$  \\
...
%		Ratio of CPU time $\left(\text{MC}/ \text{MISC} \right)$  &$\red{13}$ & $\red{  17}$   \\
		\bottomrule[1.25pt]
	\end{tabular}
	\caption{Comparison of the computational time (in seconds) of the different methods coupled with Richardson extrapolation (level $2$), to compute the call option price of the rBergomi model for different numbers of time steps. The average MC CPU time is computed over $100$ runs.}
...
\begin{figure}[htb]
	...
	\begin{subfigure}{0.33\textwidth}
		...
		\label{fig:3}
	\end{subfigure}
	\caption{\red{Comparing the numerical complexity of the different  methods with the different configurations in terms of the level of Richardson extrapolation. a) MC methods. b) QMC methods. d) ASGQ methods.} }
...
\begin{figure}[h!]
	...
	\caption{\red{Computational work comparison for the different methods with the best configurations concluded from Figure \ref{fig: Comparing the numerical complexity of the different  methods with the different configurations}}, for the case of parameter set $1$ in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}. \red{This plot shows that to achieve a relative error below $1\%$, ASGQ coupled with level $2$ of Richardson extrapolation and QMC coupled with level $1$ of  Richardson extrapolation have the same performance. Furthermore, they outperform significantly MC method coupled with level $1$ of Richardson extrapolation}.}
...
\hline
				QMC + smooth   & $\underset{(0.02,0.02)}{\mathbf{0.04}}$  &  $\underset{(0.008,0.009)}{\mathbf{0.017}}$  & $\underset{(0.004,0.004)}{\mathbf{0.008}}$ & $\underset{(0.001,0.001)}{\mathbf{\red{0.002}}}$  \\	
			M(\# QMC samples) 	& $2^3 \times 2^9=4096$  &  $2^3 \times 2^{10}=    8192$  & $2^3 \times 2^{12}=  32768$ & $2^3 \times 2^{15}=
			262144$ \\
...
%		$  \\	
\hline
		QMC method+ smooth   & $ 0.3$  & $ 0.7$  & $ 3.25$ & $ \red{27}$  \\	
		\hline
		QMC method+ non smooth   & $ 0.9$  & $ 1.8$  & $ 7$ & $ 55$  \\	
		\hline
		MC method   & $ 0.6$  & $  6.4$  & $  66$ & $ \red{1976}$  \\	
%		\hline
...
\FloatBarrier
	...
	\caption{Computational work comparison for   the different  methods, for the case of parameter set $2$ in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}. \red{This plot shows that to achieve a relative error below $1\%$, ASGQ and QMC have similar performance and they outperform significantly  MC method in terms of computational time}.}
...
%		M(\# MC samples) 	& $5 \times 10^6$  & $5 \times 10^6$  & $5 \times 10^6$ & $5 \times 10^6$  \\
		QMC + smooth    & $\underset{(0.006,0.009)}{\mathbf{0.015}}$  & $\underset{(0.004,0.004)}{ \mathbf{0.008}}$  & $\underset{(0.003,0.0036)}{\mathbf{0.0066}}$ & $\underset{(0.002,0.002)}{\mathbf{\red{0.004}}}$  \\	
...
%		$  \\	
	\hline
		QMC method + smooth   & $0.65$  & $ 1.4$  & $  3.25$ & $ \red{7.5}
		$  \\	
			\hline
		QMC method + non smooth   & $0.9$  & $ 1.8$  & $  3.6$ & $ 15
		$  \\	
		\hline
		MC method   & $4$  & $ 12$  & $  40$ & $ \red{160}
		$  \\	
		\bottomrule[1.25pt]
...
%			M(\# MC samples)   & $5 \times 10^6$  & $5 \times 10^6$  & $5 \times 10^6$ & $5 \times 10^6$  \\		
			...
			\hline
		QMC  + smooth   & $\underset{(0.07,0.085)}{\mathbf{0.155}}$  & $\underset{(0.03,0.04)}{\mathbf{0.07}}$  & $\underset{(0.02,0.019)}{\mathbf{0.039}}$ & $\underset{(0.01,0.01)}{\mathbf{\red{0.02}}}$  \\		
...
%		$  \\	
		\hline
		QMC method + smooth   & $ 0.17 $  & $  0.35$  & $ 1.6$ & $ \red{4}$  \\	
		\hline
			QMC method + non smooth   & $ 0.45 $  & $  1.8$  & $ 3.6$ & $ 8$  \\	
		\hline
		MC method   & $ 0.08 $  & $  0.6$  & $ 5.2$ & $ \red{40}$  \\	
	\bottomrule[1.25pt]
...
%		\hline
	\end{tabular}
	\caption{Comparison of the computational time (in seconds) of the different methods, to compute the call option price of rBergomi model for different numbers of time steps. The average  MC CPU time is computed over $100$ runs. The values marked in red correspond to the values used for computational work comparison against MC method, reported in Table \ref{table:Summary of our numerical results.}. }
