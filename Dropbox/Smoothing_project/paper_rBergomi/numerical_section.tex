In this section, we show the results obtained through the different numerical experiments,  conducted across different parameter constellations for the rBergomi model. Details about these examples are presented in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}. The first set is the one that is closest to the empirical findings \cite{gatheral2014volatility,bennedsen2016decoupling}, which suggest that $H \approx 0.1$ (The choice of parameters values of $\nu= 1.9$ and $\rho=-0.9$ is justified by \cite{bayer2016pricing}, where it is shown that these values are remarkably consistent with the SPX market on $4$th February $2010$). For the remaining $3$ sets in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}, we wanted to test the potential of our method for a very rough case, that is $H=0.02$, for $3$ different  scenarios  of moneyness. It is interesting to note that due to the poor behavior of the strong error, that is of order of the Hurst parameter  $H$ \cite{neuenkirch2016order}, we expect that variance reduction methods, such as MLMC, to be inefficient for very small values of $H$. We emphasize that we checked the robustness of our method for other parameter sets, but for illustration purposes, we just show results for parameters sets presented in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}.
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  & $\underset{(0.54,0.15)}{\mathbf{
			\red{0.69}}}$& $ \underset{(0.29,0.13)}{\mathbf{    
			\red{0.42}}}$ & $ \underset{(0.15,0.16)}{\mathbf{     
			0.31
		...
		MISC ($\text{TOL}_{\text{MISC}}=10^{-2}$)  & $\underset{(0.54,0.12)}{\mathbf{ 
			0.66}}$ & $ \underset{(0.29,6e-04)}{\mathbf{  \red{0.29}}}$ & $\underset{(0.15,0.01)}{\mathbf{    0.16}}$&  $ \underset{(0.07,0.01)}{\mathbf{\red{0.08}}}$  \\
...
%		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-1}$)  & $0.08$ & $0.13$ & $0.2$ \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  & $\red{0.08}$ & $\red{0.13}$ & $0.7$  & $163$ \\
%		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-2}$)  & $0.08$ & $0.25$ & $7$   \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-2}$)  & $0.2$& $\red{5}$ & $\red{333}$ &  $\red{1602}$\\
...
%		\hline
	\end{tabular}
	\caption{Comparison of the computational time (in Seconds) of  MC and MISC, to compute call option price of the rBergomi model for different number of time steps. The average MC CPU time is computed over $100$ runs.}
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		Method \textbackslash  Steps            & $1-2$ & $2-4$ & $4-8$  \\
		\hline
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  & $\underset{(0.96,0.37)}{\mathbf{  \red{1.33}}}$ & $\underset{(0.07,0.11)}{\mathbf{\red{0.18}}}$ & $\underset{(0.015,0.129)}{\mathbf{0.144}}$   \\
...
%		M(\# MC samples) & $3 \times 10^4$  & $2 \times 10^5$ & $4 \times 10^4$  \\
		...
		MC &$\underset{(0.96,0.92)}{\mathbf{1.88}}$  & $\underset{(0.07,0.07)}{\mathbf{0.14}}$ & $\underset{(0.015,0.015)}{\mathbf{0.03}}$  \\
...
%		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-1}$)  & $0.1$ & $0.18$ & $0.3$   \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  & $\red{0.1}$ & $\red{0.2}$ & $1.6$  \\
...
%		$  \\
		\hline
		\end{tabular}
		\caption{Comparison of the computational time (in Seconds) of  MC and MISC, using Richardson extrapolation (level $1$), to compute call option price of the rBergomi model for different number of time steps. The average MC CPU time is computed over $100$ runs.}
...
\begin{table}[!h]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		}}$ & $\underset{(0.006,0.107)}{\mathbf{ \red{0.113}}}$ \\
		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-2}$)  & $\underset{(0.24,0.25)}{\mathbf{   \red{0.49}
...
%			M(\# MC samples) & $5 \times 10^4$  & $7 \times 10^5$   \\
				...
				\hline
		MC   & $\underset{(0.24,0.21)}{\mathbf{0.45}}$  & $\underset{(0.006,0.006)}{\mathbf{0.012}}$    \\
			M(\# MC samples) & $4 \times 10^2$  & $2 \times 10^5$   \\
		\hline
	\end{tabular}
...
\begin{table}[!h]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  & $0.2$ & $\red{2}$ &   \\
		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-2}$)  & $\red{0.5}$ & $\red{74}$  \\
...
%		Ratio of CPU time $\left(\text{MC}/ \text{MISC} \right)$  &$\red{13}$ & $\red{  17}$   \\
		\hline
	\end{tabular}
	\caption{Comparison of the computational time (in Seconds) of  MC and MISC, using Richardson extrapolation (level $2$), to compute call option price of the rBergomi model for different number of time steps. The 	average MC CPU time is computed over $100$ runs.}
...
\begin{figure}[h!]
	...
	\caption{Computational work comparison for  MISC and MC (with and without) Richardson extrapolation.}
...
For the case of set $2$, we just conduct our numerical experiments for the case without Richardson extrapolation, since the results show that we meet a small enough error tolerance without the need to apply   Richardson extrapolation. Our numerical experiments show that MISC is  $5$ times faster than MC, to achieve  a total relative error around $0.2\%$ (see Tables \ref{Comparsion of the computational time of  MC and MISC, used to compute Call option price of rBergomi model for different number of time steps. Case set3} and \ref{Total error of MISC and MC to compute Call option price of the different tolerances for different number of time steps. Case set 3, without Richardson extrapolation. The numbers between parentheses are the corresponding absolute errors.}).  We note that the values marked in red correspond to the total relative errors plotted in Figure \ref{fig:Complexity plot for  MISC for case set $3$ parameters, comparison}, and will be used for computational work comparison against MC.
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		\hline
%%
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  &  $\underset{(0.02,0.01)}{\mathbf{\red{0.03}}}$ & $\underset{(0.008,0.014)}{\mathbf{0.022}}$& $\underset{(0.004,0.018)}{\mathbf{ 0.022}}$ & $\underset{(0.001,0.016)}{\mathbf{ 0.017}}$   \\

		MISC ($\text{TOL}_{\text{MISC}}=10^{-2}$)  &  $\underset{(0.02,0.01)}{\mathbf{0.03}}$ & $\underset{(0.008,0.009)}{\mathbf{\red{0.017}}}$& $\underset{(0.004,0.004)}{\mathbf{ 0.008}}$ & $\underset{(0.001,4e-04)}{\mathbf{ \red{0.001}}}$  \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-3}$)  &  $\underset{(0.02,8e-04)}{\mathbf{0.02}}$ & $\underset{(0.008,8e-04)}{\mathbf{\red{0.009}}}$& $\underset{(0.004,8e-04)}{\mathbf{\red{0.005}}}$  & $\underset{(0.001,4e-04)}{\mathbf{ 0.001}}$  \\
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		\hline
%		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-1}$)  & $0.1$ & $0.1$ & $0.2$ & $0.4$  \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  & $\red{0.1}$ & $0.1$ & $0.2$ & $0.8$ \\
%		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-2}$)  & $0.1$ & $0.1$ & $0.2$ & $22$  \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-2}$)  & $0.1$ & $\red{0.5}$ & $8$ & $\red{92}$ \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-3}$)  & $0.5$ & $\red{3}$ & $\red{24}$ & $226$ \\
...
\FloatBarrier
	...
	\caption{Computational work comparison for  MISC  and MC.}
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  &  $\underset{(0.006,0.002)}{\mathbf{\red{0.008}}}$ & $\underset{(0.004,0.005)}{\mathbf{0.009}}$& $\underset{(0.003,0.005)}{\mathbf{ 0.008}}$ & $\underset{(0.002,0.007)}{\mathbf{ 0.009}}$   \\
		...
		MISC ($\text{TOL}_{\text{MISC}}=10^{-3}$)  &  $\underset{(0.006,0.002)}{\mathbf{0.008}}$& $\underset{(0.004,0.002)}{\mathbf{0.006}}$& $\underset{(0.003,1e-04)}{\mathbf{0.003}}$  & $\underset{(0.002,1e-04)}{\mathbf{ \red{0.002}}}$  \\
...
%		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-1}$)  & $0.1$ & $0.1$ & $0.1$ & $0.3$  \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  & $\red{0.1}$ & $0.1$ & $0.1$ & $1$ \\
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  & $\underset{(0.07,0.05)}{\mathbf{\red{0.09}}}$ & $\underset{(0.03,0.04)}{\mathbf{\red{0.07}}}$& $\underset{(0.02,0.05)}{\mathbf{ 0.07}}$ & $\underset{(0.01,2e-04)}{\mathbf{ 0.06}}$   \\
		...
		MISC ($\text{TOL}_{\text{MISC}}=10^{-3}$)  &  $\underset{(0.07,5e-04)}{\mathbf{0.07}}$& $\underset{(0.03,4e-04)}{\mathbf{\red{0.03}}}$& $\underset{(0.02,3e-04)}{\mathbf{0.02}}$  & $\underset{(0.01,2e-04)}{\mathbf{ 0.01}}$  \\
%
%		\hline
...
%		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-1}$)  & $0.1$ & $0.1$ & $0.2$ & $0.5$  \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-1}$)  & $\red{0.1}$ & $\red{0.1}$ & $0.2$ & $0.5$ \\
%		MISC ($\text{TOL}_{\text{MISC}}=5.10^{-2}$)  & $0.1$ & $0.1$ & $0.2$ & $5$  \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-2}$)  & $0.1$ & $0.1$ & $\red{8}$ & $97$ \\
		MISC ($\text{TOL}_{\text{MISC}}=10^{-3}$)  & $0.7$ & $\red{4}$ & $26$ & $1984$ \\
...
%		\hline
	\end{tabular}
	\caption{Comparison of the computational time (In seconds) of  MC and MISC, to compute call option price of rBergomi model for different number of time steps. The average  MC CPU time is computed over $100$ runs. }
