...
\begin{table}[!h]
	...
	\toprule[1.5pt]
		Parameter set           & Level of Richardson extrapolation    &  Relative error  & CPU time  $\left(\text{MC}/ \text{ASGQ} \right)$ & CPU time  $\left(\text{MC}/ \text{QMC} \right)$\\
		\hline
			Set $1$ & level $1$ &  $3\%$&  $1.6$ &  $6$\\	
              & level $2$ &  $1\%$ &  $9.3$ &  $6.3$ \\	
              \hline
            Set $2$  & without    &  $0.2\%$&  $5$ &  $61.8$\\		
				 \hline
					Set $3$  & without    &  $0.4\%$&  $7$ &  $10$\\	
					\hline
						Set $4$ & without  &  $2\%$&  $1.3$ &  $10$\\	
		\bottomrule[1.25pt]
	\end{tabular}
\end{small}
	\caption{Summary of relative errors and computational gains, achieved by the different methods. In this table, we highlight the computational gains achieved by ASGQ and QMC over MC method to meet a certain error tolerance. As expected, these gains are improved when applying Richardson extrapolation as observed for the case of parameters set $1$. We provide details about the way we compute these gains for each case in the following sections.}
...
%		\hline
	\end{tabular}
	\caption{Comparison of the computational time (in seconds) of  MC and ASGQ, to compute the call option price of the rBergomi model for different numbers of time steps. The average MC CPU time is computed over $100$ runs.}
...
\FloatBarrier
	...
	\caption{Computational work comparison for  ASGQ  and MC methods, for the case of parameter set $2$ in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}. This plot shows that to achieve a relative error below $1\%$, ASGQ outperforms MC method in terms of computational time.}
