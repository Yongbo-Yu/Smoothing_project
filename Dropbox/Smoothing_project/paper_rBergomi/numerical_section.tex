...
\subsection{Comparing the different  errors and computational time for MC and \red{ASGQ}}\label{sec:Comparing different  errors and complexity for MC and MISC}
In this section, we conduct a comparison between MC and \red{ASGQ} in terms of errors and computational time. We show tables and plots reporting  the different relative errors involved in the MC method (bias and statistical error\footnote{The statistical error estimate of MC is  $C_{\alpha} \frac{\sigma_M}{\sqrt{M}}$, where $M$ is the number of samples and $C_{\alpha}=1.96$ for $95\%$ confidence interval.}  estimates), and in \red{ASGQ} (bias and quadrature error estimates).  While fixing  a  sufficiently small error tolerance in the price estimates,  we also compare the computational time needed for both methods to meet the desired error tolerance.  We note that  in all cases the actual work (runtime) is obtained using an Intel(R) Xeon(R) CPU E$5$-$268$ architecture. 
...
\item[ii)] The estimated  biased solution,  $C_{RB}^{N}$, is used as a reference solution  to \red{the ASGQ method} to compute the quadrature error, $\mathcal{E}_Q(\text{TOL}_{\text{ASGQ}},N)$, defined by \eqref{eq:quadrature error}.
...
\begin{table}[!h]
	...
	\end{tabular}
\end{small}
	\caption{Summary of relative errors and computational gains, achieved by the different methods. In this table, we highlight the computational gains achieved by \red{ASGQ} over MC method to meet a certain error tolerance. As expected, these gains are improved when applying Richardson extrapolation as observed for the case of parameters set $1$. We provide details about the way we compute these gains for each case in the following sections.}
...
\hline
				QMC    & $\underset{(0.54,0.59)}{\mathbf{1.13}}$  & $\underset{(0.295,0.34)}{\mathbf{0.635}}$  &$\underset{(0.155,0.19)}{
		...
		\bottomrule[1.25pt]
	\end{tabular}
	\caption{Total relative error of \red{ASGQ}, without Richardson extrapolation, with different tolerances, and MC to compute the call option prices for different numbers of time steps. The values between parentheses correspond to the different errors contributing to the total relative error: for \red{ASGQ} we report the bias and quadrature errors and for MC we report the bias and the statistical errors estimates. The number of MC samples, $M$, is chosen to satisfy \eqref{optimal_number_samples}.}
...
%		\hline
	\end{tabular}
	\caption{Comparison of the computational time (in seconds) of  MC and \red{ASGQ}, to compute the call option price of the rBergomi model for different numbers of time steps. The average MC CPU time is computed over $100$ runs.}
...
%		M(\# MC samples) & $3 \times 10^4$  & $2 \times 10^5$ & $4 \times 10^4$  \\
		...
		QMC &$\underset{(0.96,0.63)}{\mathbf{1.59}}$  & $\underset{(0.07,0.07)}{\mathbf{0.14}}$ & $\underset{(0.015,0.016)}{\mathbf{\red{0.031}}}$  \\
...
%			M(\# MC samples) & $5 \times 10^4$  & $7 \times 10^5$   \\
				...
				\hline
		QMC   & $\underset{(0.24,0.26)}{\mathbf{0.5}}$  & $\underset{(0.006,0.008)}{\mathbf{\red{0.014}}}$    \\
...
\begin{figure}[h!]
	...
	\caption{Computational work comparison for \red{ASGQ} and MC methods (with and without) Richardson extrapolation, for the case of parameter set $1$ in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}. This plot shows that to achieve a relative error below $1\%$, \red{ASGQ} coupled with level $2$ of Richardson extrapolation is the best option in terms of computational time. Furthermore, applying Richardson extrapolation brings a significant improvement for \red{ASGQ} and MC methods, in terms of numerical complexity.}
...
\hline
				QMC    & $\underset{(0.02,0.02)}{\mathbf{0.04}}$  &  $\underset{(0.008,0.008)}{\mathbf{0.016}}$  & $\underset{(0.004,0.005)}{\mathbf{0.009}}$ & $\underset{(0.001,0.0015)}{\mathbf{\red{0.0025}}}$  \\	
			M(\# QMC samples) 	& $2^3 \times 2^8=2048$  &  $2^3 \times 2^{10}=    8192$  & $2^3 \times 2^{11}= 16384$ & $2^3 \times 2^{13}=
       65536$ \\
			\hline
...
%		$  \\	
\hline
		QMC method   & $ 0.2$  & $ 0.7$  & $ 1.6$ & $ \red{8}$  \\	
		\hline
		MC method   & $ 0.15$  & $  1.6$  & $  16.5$ & $ \red{494}$  \\	
%		\hline
...
%		$  \\	
		...
		\bottomrule[1.25pt]
	\end{tabular}
	\caption{Comparison of the computational time (in seconds) of  MC and \red{ASGQ}, to compute the call option price of the rBergomi model for different numbers of time steps. The average  MC CPU time is computed over $100$ runs. The values marked in red correspond to the values used for computational work comparison against MC method, reported in Table \ref{table:Summary of our numerical results.}. }
...
\FloatBarrier
	...
	\caption{Computational work comparison for  \red{ASGQ}  and MC methods, for the case of parameter set $2$ in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}. This plot shows that to achieve a relative error below $1\%$, \red{ASGQ} outperforms MC method in terms of computational time.}
...
%		M(\# MC samples) 	& $5 \times 10^6$  & $5 \times 10^6$  & $5 \times 10^6$ & $5 \times 10^6$  \\
		QMC    & $\underset{(0.006,0.006)}{\mathbf{0.012}}$  & $\underset{(0.004,0.0046)}{ \mathbf{0.0086}}$  & $\underset{(0.003,0.0032)}{\mathbf{0.0062}}$ & $\underset{(0.002,0.0018)}{\mathbf{\red{0.0038}}}$  \\	
...
\FloatBarrier
	...
	\caption{Comparison of computational work for MC and \red{ASGQ} methods, for the case of parameter set $3$ in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}. This plot shows that to achieve a relative error below $1\%$, \red{ASGQ} outperforms MC method in terms of computational time.}
...
%			M(\# MC samples)   & $5 \times 10^6$  & $5 \times 10^6$  & $5 \times 10^6$ & $5 \times 10^6$  \\		
			...
			\hline
		QMC    & $\underset{(0.07,0.065)}{\mathbf{0.135}}$  & $\underset{(0.03,0.03)}{\mathbf{0.67}}$  & $\underset{(0.02,0.024)}{\mathbf{0.044}}$ & $\underset{(0.01,0.012)}{\mathbf{\red{0.022}}}$  \\		
...
%		$  \\	
		\hline
		QMC method   & $ 0.08 $  & $  0.17$  & $ 0.4$ & $ \red{1}$  \\	
		\hline
		MC method   & $ 0.02 $  & $  0.15$  & $ 1.4$ & $ \red{10}$  \\	
	\bottomrule[1.25pt]
...
%		\hline
	\end{tabular}
	\caption{Comparison of the computational time (in seconds) of  MC and \red{ASGQ}, to compute the call option price of rBergomi model for different numbers of time steps. The average  MC CPU time is computed over $100$ runs. The values marked in red correspond to the values used for computational work comparison against MC method, reported in Table \ref{table:Summary of our numerical results.}. }
...
\FloatBarrier
	...
	\caption{Comparison of computational work for MC and \red{ASGQ} methods, for the case of parameter set $4$ in Table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}. This plot shows that to achieve a relative error around $1\%$, \red{ASGQ} and  MC methods have similar performance in terms of computational time.}
