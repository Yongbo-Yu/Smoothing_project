...
We show in Sections (\ref{sec:Case of set $2$ parameters_linear}, \ref{sec:Case of set 3 parameters},\ref{sec:Case of set 4 parameters},\ref{sec:Case of set 5 parameters}) the detailed outputs for each case of parameter set as in table \ref{table:Reference solution, using MC with $500$ time steps, of Call option price under rBergomi model, for different parameter constellation.}. The conclusions of this section are: 
...
We report in table \ref{Total error of MISC and MC to compute Call option price of the different tolerances for different number of time steps. Case $K=1$, $H=0.07$, without Richardson extrapolation. The numbers between parentheses are the corresponding absolute errors,linear} the total relative error which is the sum of the relative bias and statistical error for MC, and the relative quadrature error for MISC. We note that we used a number of samples for MC such that the relative statistical error \footnote{The statistical error of MC is  $\frac{\sigma_M}{\sqrt{M}}$, where $M$ is the number of samples and $\sigma_M$ is the standard deviation of the MC estimator.} is almost equal to the stable relative  quadrature error, in order to have a fair complexity comparison between the two methods. 
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		MC    & $\underset{(0.54,3e-04)}{\mathbf{0.54}}$  & $\underset{(0.29,6e-04)}{\mathbf{0.29}}$  &$\underset{(0.15,0.01)}{
				\mathbf{0.16}}$  \\	
		M(\# MC samples)   & $8.10^6$  & $8.10^6$  &$10^5$  \\	
		\hline
	\end{tabular}
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		M(\# MC samples) & $4.10^4$  & $2.10^5$ & $5.10^4$  \\
...
%		MISC ($\text{TOL}_{\text{MISC}}=10^{-4}$)  &  $\underset{(0.02,8e-04)}{\mathbf{\red{0.02}}}$ & $\underset{(0.008,8e-04)}{\mathbf{\red{0.009}}}$& $\underset{(0.004,8e-04)}{\mathbf{0.005}}$ & $\mathbf{ -}$ \\
		...
		MC    & $\underset{(0.02,8e-04)}{\mathbf{0.02}}$  &  $\underset{(0.008,8e-04)}{\mathbf{0.009}}$  & $\underset{(0.004,8e-04)}{\mathbf{0.005}}$ & $\underset{(0.001,4e-04)}{\mathbf{0.001}}$  \\	
			M(\# MC samples) 	& $5.10^6$  &  $5.10^6$  & $5.10^6$ & $5.10^6$  \\
		\hline
	\end{tabular}
...
\begin{table}[h!]
	...
	\hline
		...
		MC    & $\underset{(0.003,0.003)}{\mathbf{0.007}}$  &   $\underset{(0.001,0.002)}{\mathbf{0.003}}$  &  $\underset{(0.001,0.001)}{\mathbf{0.002}}$  \\
			M(\# MC samples) & $5.10^4$  &   $4.10^5$  &  $2.10^6$  \\
		\hline
	\end{tabular}
