...
\red{Maybe criterion given by \eqref{optimality_criterion_2} is not the right choice since decomposition \eqref{eq:smoothing_decomposition} should leave the distribution, and hence the variance invariant.}
...
\textbf{Assumption $1$:} We assume that the first term in the right-hand side of \eqref{eq:SDE_decomposition_componentwise_exapanded}  is the dominant term compared to the remaining terms.

\red{We need to formulate clearly what we mean by this statement. Also I see some issues with this  assumption, since it implicitly implies from \eqref{eq:smoothing_decomposition_componentwise} that 
$\mathbf{Z} \approx Z_v \mathbf{v}$, which is already a strong assumption about the solution $v$. As discussed with Raul, the approximation should be done in terms of the variance of $g$. I have suggest a counter-example but using local terms, but Raul suggest to look at the global terms which seems to validate the assumption $1$!}.
...
\end{align*}
where $p= \mathbb{P}(\phi(\widehat{X}_T) \ge 0)$, and can be approximated using the normal approximation based on the moment expansion where the first and second moments are provided by  \ref{eq:first_moment_approximation} and \ref{eq:second_moment_approximation}. Then once we have an approximation of $p$, we can optimize over $v$.
\red{Add details here for the optimization problem \eqref{optimality_criterion_2} for the call and the binary examples.}
\end{example} 
\begin{example}[Call option]
We consider the case of one dimensional call omption,  $g$ is given by \eqref{eq:call_option} and $\phi(x)=x-K$, where $K$ is the strike price. 
\end{example} 
