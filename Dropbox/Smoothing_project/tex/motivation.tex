...
$\mathbf{Z} \approx Z_v \mathbf{v}$, which is already a strong assumption about the solution $v$. As discussed with Raul, the approximation should be done in terms of the variance of $g$. I have suggest a counter-example but using local error, but Raul suggest to look at the global error term!}), and denote the approximate process $\hat{X}$, whose dynamics are given by
...
\begin{align}\label{polynomial_kink_location_derivative_basket}
	P^\prime(W_1)=\sum_{j=1}^{d} c_j X_0^{(j)}  \frac{\sigma^{(j)} \Delta t A^{\text{inv}}_{j1}} {\sqrt{T}} \left( \prod_{i=0}^{N-1} g_i^{(j)}(Y_1) \right) \left[ \sum_{i=0}^{N-1} \frac{1}{g_i^{(j)}(Y_1)}\right].
\end{align}
