...
\newpage
\subsubsection*{With Richardson extrapolation (level 1)}
From figure \ref{fig:Weak_rate_H_007_with_rich_change_measure} , we see that  for $H=0.07$, we get a weak error of order $\Delta t^2$. The upper and lower bounds are $95\%$ confidence interval. However, comparing to figure \ref{fig:Weak_rate_H_007_with_rich}, I think we have issue since we got worser weak error values after the change of measure. We need to double check this stage.
...
\subsubsection*{Case $H=0.07$, Relative error for different methods}
\label{sec:Case $H=0.07$, Relative error for different methods_change_measure}
\begin{table}[h!]
	\centering
	\begin{tabular}{l*{6}{c}r}
		Method \textbackslash  Steps            & $2$ & $4$ & $8$ & $16$ &   \\
...
\subsubsection{Plotting the Richardson integrand for the change of measure}
