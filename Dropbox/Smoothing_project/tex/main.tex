...
&=\max\left(\sum_{i=1}^{d} w_i \exp(y_i)  h_i( Z_2, \dots,Z_N, Z_{N+1}, \dots,Z_{2N},\dots, Z_{(d-1)N+2},\dots,Z_{dN}, \dots,Z_N)-K,0\right)
 \COMMA
\end{align}
where $w_i=c_i X^{(i)}_0 e^{-\frac{\sigma_i^2}{2} T}$, $y_i=\sum_{\ell=1}^{d}C_{(N-1) d +i,(\ell-1)N+1} Z_{(\ell-1)N+1}$ ($y_i$ can be seen always as a linear combination including the first factors in each asset dimension).


\subsubsection{Setting $2$}

The fourth example that we test is basket option under BS model where the process $\mathbf{X}$ is the discretized $d$-dimensional Black-Scholes model and the payoff function $g$ is given by
\begin{align}
...
\end{table}
	...
	\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\linewidth]{./figures/Basket_2d_MISC_quadrature_error/relative_quad_error_wrt_MISC_TOL_non_rich}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\linewidth]{./figures/Basket_2d_Complexity_rates/error_vs_time}
