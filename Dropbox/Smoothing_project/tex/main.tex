...
We get the kink point by running Newton iteration in each dimension seperately for root solving of each polynomial $P^{(j)}$ as expressed in \ref{polynomial_kink_location_basket} with a precision of $10^{-10}$. We  decompose the total integration domain $\Omega$  into sub-domains $\Omega_i,\: i=1,2,\dots, 2^d$ such that the integrand is smooth in the interior of  $\Omega_i$ and such that the kink is located along the boundary of these areas. The total integral is then given as the sum of the separate integrals, \ie
\begin{align}
	h(\mathbf{z}^{(1)}_{-1},\dots,\mathbf{z}^{(d)}_{-1}) &:=  \int_{\Omega}  \max  \left[ \left(\sum_{j=1}^{d} \Phi \circ \Psi(T;z_1^{(j)},\mathbf{z}^{(j)}_{-1})\right)-K,0\right]   \rho_d(z_1^{(1)},\dots,z_1^{(d)}) dz_1^{(1)}\dots dz_1^{(d)}  \nonumber\\
...
\FloatBarrier
\subsubsection{Weak error plots for $2$-dimensional Basket call option} \label{sec:Weak error plots_Basket_2D_call}

We consider the case of $2$-dimensional Basket call option, with parameters: $S^{(1,2)}=100$, $K=100$, $\sigma^{(1,2)}=0.2$, $\rho=0.3$, $r=0$, $T=1$.  The MC  value of this case (with large number of samples) is $6.437949$ with a statistical error equal to $	0.000328$.


In this section, we include the results of weak error rates for $2$ scenarios, without/with Richardson extrapolation (level $1$). We note that the weak errors plotted here correspond to relative errors.  We note that in order to get accurate estimates of the bias, we needed to kill the laguerre quadrature error by increasing $\beta$ ($\beta$: the number of quadrature points used for Laguere to get $h(\mathbf{z}_{-1})$). For illustration, we show to cases $\beta=32$ and $\beta=64$. We mention that from our numerical experiences that the bias did not change as we increase $\beta$ greater than $64$ points.

Focusing on the results produced by seeting $\beta=64$ (which gives the right behavior to be observed for the bias), we can see from figure \ref{fig:Weak_rate_call_without_rich_beta_64} that we get a weak error of order $\Delta t$ for the case without Richardson extrapolation. From figure \ref{fig:Weak_rate_call_with_rich_level1_beta_64}, we observe an improvement in the rate and the constant when using level $1$ of Richardson extrapolation, approximately of order $\Delta t^2$. For all the plots below, the upper and lower bounds are $95\%$ confidence interval.

\subsubsection*{$\beta=16$}
\begin{figure}[h!]
	\centering
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/weak_error_rates_basket_2d/without_richardson/Beta_16/weak_convergence_order_basket_option_2d_1_relative_M_10_5_beta_16}
		\caption{}
		\label{fig:sub3}
	\end{subfigure}%
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/weak_error_rates_basket_2d/without_richardson/Beta_16/weak_convergence_order_differences_basket_option_2d_1_relative_M_10_5_beta_16}
		\caption{}
		\label{fig:sub4}
	\end{subfigure}
	
	\caption{The rate of convergence of the weak error for the Basket ($2$-dimensional) call option, without Richardson extraploation, using MC with $M=10^5$: a) $\abs{\expt{g(X_{\Delta t})}-g(X)}$  b) $\abs{\expt{g(X_{\Delta t})-g(X_{\Delta t/2})}}$ }
	\label{fig:Weak_rate_basket_2d_without_rich_beta_16}
\end{figure}
...
\begin{figure}[h!]
	...
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/weak_error_rates_basket_2d/without_richardson/Beta_32/weak_convergence_order_basket_option_2d_1_relative_M_5_10_4_beta_32}
		\caption{}
		\label{fig:sub3}
	\end{subfigure}%
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/weak_error_rates_basket_2d/without_richardson/Beta_32/weak_convergence_order_differences_basket_option_2d_1_relative_M_5_10_4_beta_32}
		\caption{}
		\label{fig:sub4}
	\end{subfigure}
	
	\caption{The rate of convergence of the weak error for the Basket ($2$-dimensional) call option, without Richardson extraploation, using MC with $5.M=10^4$: a) $\abs{\expt{g(X_{\Delta t})}-g(X)}$  b) $\abs{\expt{g(X_{\Delta t})-g(X_{\Delta t/2})}}$ }
	\label{fig:Weak_rate_basket_2d_without_rich_beta_32}
\end{figure}

\subsubsection*{$\beta=64$}

\begin{figure}[h!]
	\centering
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/weak_error_rates_basket_2d/without_richardson/Beta_64/weak_convergence_order_basket_option_2d_1_relative_M_5_10_4_beta_64}
		\caption{}
		\label{fig:sub3}
	\end{subfigure}%
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/weak_error_rates_basket_2d/without_richardson/Beta_64/weak_convergence_order_differences_basket_option_2d_1_relative_M_5_10_4_beta_64}
		\caption{}
		\label{fig:sub4}
	\end{subfigure}
	
	\caption{The rate of convergence of the weak error for the Basket ($2$-dimensional) call option, without Richardson extraploation, using MC with $5.M=10^4$: a) $\abs{\expt{g(X_{\Delta t})}-g(X)}$  b) $\abs{\expt{g(X_{\Delta t})-g(X_{\Delta t/2})}}$ }
	\label{fig:Weak_rate_basket_2d_without_rich_beta_64}
\end{figure}
...
\subsubsection*{Without Richardson extrapolation}
\subsubsection*{$\beta=8$}
\begin{table}[h!]
	\centering
	\begin{tabular}{l*{6}{c}r}
		Method \textbackslash  Steps            & $2$ & $4$ & $8$ & $16$ &   \\
		\hline
		MISC ($TOl=5.10^{-1},\beta=8$)  & $  6.3988$ & $ 6.3726$ & $
		6.3684$ & $ -
		$  \\
		MISC ($TOl=10^{-1},\beta=8$)  & $  6.3988$ & $
		6.3897$ & $ 6.3849
		$ &$-$  \\
		MISC ($TOl=5.10^{-2},\beta=8$) & $  6.3988$ & $
		6.3938$ & $ 6.3869
		$ & $    -$  \\
		MISC ($TOl=10^{-2},\beta=8$) & $ 6.4055$ & $   6.3969$ & $6.3880$ &$-$  \\
		MISC ($TOl=10^{-3},\beta=8$) &  $ 6.4055$ & $    6.3969$ & $-$ & $-$  \\
			MISC ($TOl=10^{-4},\beta=8$) &  $ 6.4055$ & $    6.3969$& $-$ & $-$  \\
		\hline
		MC method ($M=10^{7}$)   & $ - $ & $  -$ & $
	$ & $  $  \\
		\hline
	...
	\label{table:Basket 2d option price of the different methods for different number of time steps, without Richardson extrapolation, beta_8}
\end{table}
...
Method \textbackslash  Steps            & $2$ & $4$ & $8$ & $16$ &   \\
\hline
MISC ($TOl=5.10^{-1}$) & $2.4$ & $8$ & $110$ & $-$  \\
MISC ($TOl=10^{-1}$)  & $2.4$ & $37$ & $137$ & $-$  \\
MISC ($TOl=5.10^{-2}$)  & $2.4$ & $42$ & $203$ & $-$  \\
MISC ($TOl=10^{-2}$)  & $5$ & $45$ & $970$ & $-$  \\
MISC ($TOl=10^{-3}$)   & $5$ & $126$ & $-$ & $-$  \\
MISC ($TOl=10^{-4}$)   & $5$ & $191$ & $-$ & $-$  \\
\hline
MC method +root finding   & $\red{-}$ & $\red{-}$ & $\red{-}$ & $\red{-}$  \\
MC method & $\red{-}$ & $\red{-}$ & $\red{-}$ & $\red{-
}$  \\
\hline

Ratio of	$\text{(MC+root finding)}/\text{(MISC)}$ & $\red{-}$ & $\red{  -}$ & $\red{-}$ & $\red{ -}$  \\
Ratio of	$(\text{MC})/(\text{MISC})$ & $\red{ -}$ & $\red{ -}$ & $\red{-}$ & $\red{ -}$  \\
\hline
\end{tabular}
...
\label{Comparsion of the computational time of  MC and MISC, used to compute Basket 2d Call option price  for different number of time steps, without Richardson extrapolation, beta_8}
\end{table}
...
$ & $6.4574
$ & $-$  \\
MISC ($TOl=10^{-2},\beta=16$) & $  6.4770$ & $ 6.4683$ & $  6.4592
$ &$-$  \\
MISC ($TOl=10^{-3},\beta=16$) & $  6.4770$ & $6.4683$ & $-$ & $-$  \\
MISC ($TOl=10^{-4},\beta=16$) & $  6.4770$ & $-$ & $-$ & $-$  \\
\hline
MC method ($M=10^{7}$)   & $  -
$ & $ -$ & $
$ & $  $  \\
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		Method \textbackslash  Steps            & $2$ & $4$ & $8$ & $16$ &   \\
		\hline
		MISC ($TOl=5.10^{-1}$) & $5$ & $17$ & $260$ & $1765$  \\
		MISC ($TOl=10^{-1}$)  & $5$ & $61$ & $372$ & $-$  \\
		MISC ($TOl=5.10^{-2}$)  & $5$ & $74$ & $395$ & $-$  \\
		MISC ($TOl=10^{-2}$)  & $10$ & $85$ & $1420$ & $-$  \\
		MISC ($TOl=10^{-3}$)   & $10$ & $205$ & $-$ & $-$  \\
			MISC ($TOl=10^{-4}$)   & $10$ & $-$ & $-$ & $-$  \\
		\hline
		MC method +root finding   & $\red{-}$ & $\red{-}$ & $\red{-}$ & $\red{-}$  \\
		MC method & $\red{-}$ & $\red{-}$ & $\red{-}$ & $\red{-
...
\begin{table}[h!]
	...
	\end{tabular}
	\caption{Call option price of the different methods for different number of time steps, with Richardson extrapolation (level $1$).}
	\label{table: Call option price of the different methods for different number of time steps, with Richardson extrapolation (level1),beta_8}
\end{table}
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		\end{tabular}
		\caption{Comparsion of the computational time of  MISC, used to compute Call option price  for different number of time steps, with Richardson extrapolation (level $1$)}
		\label{Comparsion of the computational time of  MC and MISC, used to compute Call option price  for different number of time steps, with Richardson extrapolation (level $1$),beta_8}
		\end{table}
\FloatBarrier
\subsubsection*{$\beta=16$}
