...
We have a natural error decomposition for the total error of computing the expectation in \eqref{eq: pre_integration_step_wrt_y1}, namely, $\mathcal{E}$
...
bridge (Bb)  construction \cite{moskowitz1996smoothness,caflisch1997valuation,morokoff1998generating,larcher2003tractability}, the principal component analysis (PCA)  \cite{acworth1998comparison} and the linear transformation (LT) \cite{imai2004minimizing}, etc \dots 
...
&=w^{(i)}  \exp\left(A_{i\ast} Z\right)\COMMA
\end{align}
where $w^{(i)}=X^{(i)}_0 e^{-\frac{\sigma_i^2}{2} T} $,  $A$ is a   $d \times dN$  matrix such that $A:=\left(C_{(N-1) d +i,j} \right)_{1 \le i \le d, \: 1 \le j \le dN }$, and $A_{i\ast}$ is the $i$-th row vector of A.
...
Giving the representation given by \eqref{eq:basket_call_payoff_2}, it seems to me that we need first to solve root finding problem wrt $\{y_i\}_{i=1}^d$, then giving $\{y_i^\ast\}_{i=1}^d$, we get the kink locations $\{Z_{N(i-1)+1}^\ast\}_{i=1}^d$ by solving a $d$-dimensional system. 



Another way maybe to solve the problem is to mimic the methodology in \cite{bayersmoothing}. In fact, 
...
We conduct our experiments for $3$ different examples under discretized BS model: i) single Binary, ii) single Call, and  basket call options (We just includeded results for 2-dimensional basket, I still have issue for higher dimensions that we need to discuss, and this is maybe related to the way I am solving the problem in high dimension).
