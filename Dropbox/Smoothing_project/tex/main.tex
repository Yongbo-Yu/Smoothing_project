...
In the high dimensional case, we denote by $(z_1^{(i)},\dots,z_N^{(i)})$ the $N$ Gaussian independent rdvs that will be used to construct the path of the $i$-th asset $X^{(i)}$, where $1 \le i \le d$ ($d$ denotes the number of underlyings considered in the basket). We keep the  same notations by denoting  $\psi: (z_1^{(i)},\dots,z_N^{(i)}) \rightarrow (B_1,\dots,B_N)$ the mapping of BB construction and by $\Phi: (B^{(i)}_1,\dots,B^{(i)}_N) \rightarrow X^{(i)}_T$, the mapping consisting of the time-stepping scheme. Then, we can express the option price as
...
Now,  performing the  pre-integrating step with respect to $(z_1^{(1)},\dots, z_1^{(d)} )$, results in
\begin{small}
...
In our case, and following \eqref{eq: pre_integration_step_wrt_y1_basket}, we have a multi-variate integration problem and  we have, given the previous notations,    $f:=h$, $\mathbf{Y}=(\mathbf{z}^{(1)}_{-1},\dots,\mathbf{z}^{(d)}_{-1})$, and  $\Gamma$  is finite dimensional and given by a $d (N-1)$ tensor product of intervals.  Therefore,  we define, for any multi-index $\boldsymbol{\beta}$

$$Q^{m(\boldsymbol{\beta})}: \Gamma \rightarrow \rset,\quad  Q^{m(\boldsymbol{\beta})}= \bigotimes_{n = 1}^{d (N-1)} Q^{m(\beta_n)} $$
...
A direct approximation $\expt{f[\mathbf{Y}]} \approx Q^{m(\boldsymbol{\beta})}[f]$ is not an appropriate option  due to the well-known "curse of dimensionality" effect. We use MISC as it was suggested  in \cite{haji2016multi}. MISC is a hierarchical adaptive sparse grids quadrature strategy that uses  stochastic discretizations  and classic sparsification approach to obtain an effective approximation scheme for $\expt{f}$.
 ...
 For the sake
of concreteness, in our setting from \eqref{eq: pre_integration_step_wrt_y1_basket}, we are left with a $d(N-1)$- dimensional Gaussian random inputs, which are chosen independently, resulting in  $d(N-1)$ numerical parameters for MISC, which we use as the basis of the multi-index construction. Let $l \in \{1, \ldots, d (N-1)\}$ and set
\begin{equation}
p_l \coloneqq z_{i1+1},  \quad 1 \le i \le d,\quad  (i-1)(N-1)+1\le l \le i (N-1).
\end{equation}


For a multi-index $\ell = (l_i)_{i=1}^{d(N-1)} \in \mathbb{N}^{d(N-1)}$, we denote  by
$Q_N^\ell \coloneqq Q^{N,m(\ell)}(p_{\ell})$ the result of a discretized
integral, using $N$ time steps , with parameters $p_\ell \coloneqq (p_{l_i})_{i=1}^{d(N-1)}$, and with a number of quadrature points $m(l_i)$ in the dimension $p_{l_i}$. We further define the set of
differences $\Delta Q^N_\ell$ as follows: for a single index $1 \le i \le d (N-1)$,
let
\begin{equation}
...
\begin{equation}
\Delta Q_N^\ell \coloneqq \left( \prod_{i=1}^{d(N-1)} \Delta_i \right) Q_N^\ell.
\end{equation}
...
\begin{equation}
Q_N^\infty = \sum_{l_1=0}^\infty \cdots \sum_{l_{d(N-1)} = 0}^\infty \Delta
Q_N^{(l_1, \ldots, l_{d(N-1)})} = \sum_{\ell \in \mathbb{N}^{d(N-1)}} \Delta Q_N^\ell,
\end{equation}
provided that $l_1 \to \infty$, \ldots,
$l_{d(N-1)} \to \infty$, where $Q^\infty_N$ is the biased option price, computed with $N$ time steps.
...
\mathcal{E}_Q(TOL_{\text{MISC}},N) =\abs{Q_N^\infty - Q_N^\mathcal{I}} \le \sum_{\ell \in \mathbb{N}^{d(N-1)} \setminus
	\mathcal{I}} \abs{\Delta Q_N^\ell}.
\end{equation}

If we denote the computational work at level $\ell = (l_1, \ldots, l_{d(N-1)})$, for adding an increment $\Delta Q_N^{\ell}$
...
\end{equation*}  
 ...
 In the case of Bb construction \cite{moskowitz1996smoothness,caflisch1997valuation,morokoff1998generating,larcher2003tractability}, details about the construction are given in Section \ref{sec:Brwonian bridge construction}, and  the corresponding matrix $A$, For $N=8$ is given by 
...
\subsection{The basket call option under time stepping framework}\label{sec:The basket option under time stepping framework}

\subsubsection{First way}
We consider a European Basket call option consisting of $d$ assets. The $i$-th asset $X^{(i)}$ of the basket $(i=1,\dots,d)$ is given by 
...
\begin{equation}
X^{(i)}_{T} (Z)=X^{(i)}_{N \Delta t}(Z)=X^{(i)}_0 e^{-\frac{\sigma_i^2}{2} T} \exp\left(\sum_{j=1}^{d  N} C_{(N-1) d +i,j} Z_j\right) \COMMA
\end{equation}
where $C$ is a $dN \times dN$-matrix with $C C^T=\tilde{\Sigma}:=R  \otimes \Sigma$, and $R$ is an $d \times d$-matrix with $R_{ij}=\sqrt{T/N} \rho_{ij} \sigma_i\sigma_j$. 

For notation simplification, let us denote $\mathbf{Z}^{(i)}:=(Z_{N(i-1)+1}, \dots, Z_{iN})$ and  $\mathbf{Z}^{(i)}_{-1}:=(Z_{N(i-1)+2}, \dots, Z_{iN})$. 
...
\end{align*}
 ...
 Giving the representation given by \ref{eq:basket_call_payoff_2}, it seems to me that we need first to solve root finding problem wrt $\{y_i\}_{i=1}^d$, then giving $\{y_i^\ast\}_{i=1}^d$, we get the kink locations $\{Z_{N(i-1)+1}^\ast\}_{i=1}^d$ by solving a $d$-dimensional system.  Another way maybe to solve the problem is to mimic the methodology in \cite{bayersmoothing}. However, it is not clear to me yet 


\subsubsection{Second way}

We consider the basket option under BS model where the process $\mathbf{X}$ is the discretized $d$-dimensional Black-Scholes model and the payoff function $g$ is given by
\begin{align}
...
\begin{align}\label{lognormal_dynamics_basket}
	dX^{(i)}_t=\sigma^{(i)} X^{(i)}_t dW^{(i)}_t,
\end{align}
...
\begin{align}\label{Brownian_bridge_BS_basket}
	\Delta W^{(j)}_i&=(B^{(j)}_{t_{i+1}}-B^{(j)}_{t_i})+\Delta t \frac{z_1^{(j)}}{\sqrt{T}}, \quad 1 \le j \le d \nonumber\\
	&= \Delta B^{(j)}_i + \Delta t \frac{z_1^{(j)}}{\sqrt{T}}, \quad 1 \le j \le d ,
\end{align}

implying that the numerical approximation of $X^{(j)}(T)$ satisfies
\begin{align}
...
Therefore, in this case, the integrand $h(\mathbf{z}^{(1)}_{-1},\dots,\mathbf{z}^{(d)}_{-1})$ (as expressed in \ref{eq: pre_integration_step_wrt_y1_basket}) is given by
...
\FloatBarrier
\subsubsection{Case of  $2$-dimensional Basket call option}
...
In this section, we include the results of weak error rates for $2$ scenarios, without/with Richardson extrapolation (level $1$). We note that the weak errors plotted here correspond to relative errors.  
 ...
 We can see from figure \ref{fig:Weak_rate_basket_2d_without_rich} that we get a weak error of order $\Delta t$ for the case without Richardson extrapolation. 
...
\begin{figure}[h!]
	...
	\begin{subfigure}{.35\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/weak_error_rates_basket_2d/without_richardson/weak_convergence_order_differences_basket_option_2d_1_relative_M_4_10_7_plain}
		\caption{}
		\label{fig:sub4}
	\end{subfigure}
	
	\caption{The rate of convergence of the weak error for the Basket ($2$-dimensional) call option, without Richardson extraploation, using MC with $M=4.10^7$: a) $\abs{\expt{g(X_{\Delta t})}-g(X)}$  b) $\abs{\expt{g(X_{\Delta t})-g(X_{\Delta t/2})}}$ }
	\label{fig:Weak_rate_basket_2d_without_rich}
\end{figure}
...
\begin{figure}[h!]
	...
	\caption{The rate of convergence of the weak error for the  Basket ($2$-dimensional) call option with Richardson extraploation (level 1), using MC with $M=4.10^7$: a) $\abs{\expt{2 g(X_{\Delta t/2}) -g(X_{\Delta t})}-g(X)}$  b) $\abs{\expt{3 g(X_{\Delta t/2})-g(X_{\Delta t})-2 g(X_{\Delta t/4})}}$}
...
$ & $-$  \\
MISC ($TOl=10^{-2},\beta=16$) & $  13.0548$ & $    13.0046$ & $   12.9550
$ &$-$  \\
MISC ($TOl=10^{-3},\beta=16$) & $   13.0548$ & $   13.0046$ & $-$ & $-$  \\
MISC ($TOl=10^{-4},\beta=16$) & $  13.0545$ & $   13.0047$ & $-$ & $-$  \\
MISC ($TOl=10^{-5},\beta=16$) & $  13.0545$ & $-$ & $-$ & $-$  \\
\hline
...
Method \textbackslash  Steps            & $2$ & $4$ & $8$ & $16$  \\
\hline
MISC ($TOl=5.10^{-1}$)  & $\underset{(  0.0194)}{\mathbf{    0.0015}}$  & $\underset{(   0.1392
	)}{\mathbf{    0.0108}}$ & $\underset{( 0.0447
	
	-
	)}{\mathbf{ 0.0035}}$ &$\underset{(
	0.0194)}{\mathbf{   \red{ 0.0015}}}$ \\
MISC ($TOl=10^{-1}$)  & $\underset{(  0.0194)}{\mathbf{    0.0015}}$   & $\underset{(   0.0057)}{\mathbf{4.4e-04}}$ & $\underset{(0.0255)}{\mathbf{\red{0.0020}}}$ &$\underset{(0.0037)}{\mathbf{2.9e-04}}$ \\
MISC ($TOl=5.10^{-2}$)  & $\underset{(  0.0194)}{\mathbf{    0.0015}}$  & $\underset{( 0.0080)}{\mathbf{\red{6.2e-04}}}$ & $\underset{(
	0.0249
	)}{\mathbf{0.0019}}$ &$\underset{(-)}{\mathbf{-}}$ \\
MISC ($TOl=10^{-2}$)  & $\underset{( 0.0345)}{\mathbf{\red{0.0027}}}$  & $\underset{( 0.0077)}{\mathbf{6.0e-04}}$ & $\underset{(
	0.0249
	)}{\mathbf{0.0019}}$&$\underset{(-)}{\mathbf{-}}$ \\
MISC ($TOl=10^{-3}$)  & $\underset{( 0.0345)}{\mathbf{0.0027}}$  & $\underset{( 0.0077)}{\mathbf{6.0e-04}}$ & $\underset{(
-
	)}{\mathbf{-}}$ &$\underset{(-)}{\mathbf{-}}$ \\
MISC ($TOl=10^{-4}$)  & $\underset{( 0.0342)}{\mathbf{0.0027
}}$  & $\underset{( 0.0078)}{\mathbf{6.0e-04}}$& $\underset{(
	-
	)}{\mathbf{-}}$ &$\underset{(-)}{\mathbf{-}}$ \\
MISC ($TOl=10^{-5}$)  & $\underset{( 0.0342)}{\mathbf{0.0027
}}$  & $\underset{( -)}{\mathbf{-}}$ & $\underset{(
	-
	)}{\mathbf{-}}$ &$\underset{(-)}{\mathbf{-}}$ \\
\hline
\end{tabular}
\caption{Quadrature error of MISC to compute Basket ($2$-dimensional) Call option price of the different tolerances for different number of time steps, without Richardson extrapolation. The numbers between parentheses are the corresponding absolute errors.}
...
\FloatBarrier
	...
	\caption{Relative quadrature error of MISC to compute Basket ($2$-dimensional) Call option price of the different tolerances for different number of time steps, without Richardson extrapolation.}
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		Method \textbackslash  Steps            & $2$ & $4$ & $8$ & $16$  \\
		\hline
		MISC ($TOl=5.10^{-1}$) &$\mathbf{0.0108}$ & $\mathbf{0.0183}$ & $\mathbf{0.0058}$ & $\mathbf{\red{0.0029}}$  \\	
		MISC ($TOl=10^{-1}$) & $\mathbf{0.0108}$ & $\mathbf{0.0079}$ & $\mathbf{\red{0.0043}}$ & $\mathbf{0.0017}$  \\	
		MISC ($TOl=5.10^{-2}$) & $\mathbf{0.0108}$ & $\mathbf{\red{0.0081}}$ & $\mathbf{0.0042}$ & $\mathbf{-}$  \\	
		MISC ($TOl=10^{-2}$)& $\mathbf{\red{0.0120}}$ & $\mathbf{0.0081}$ &$\mathbf{0.0042}$ & $\mathbf{-}$  \\	
		MISC ($TOl=10^{-3}$) &  $\mathbf{0.0120}$& $\mathbf{0.0081}$ & $\mathbf{-}$ & $\mathbf{-}$  \\	
		MISC ($TOl=10^{-4}$) &  $\mathbf{0.0120}$& $\mathbf{0.0081}$ & $\mathbf{-}$ & $\mathbf{-}$  \\
		MISC ($TOl=10^{-5}$) &  $\mathbf{0.0120}$& $\mathbf{-}$ & $\mathbf{-}$ & $\mathbf{-}$  \\
		\hline	
		...
		\hline
	\end{tabular}
	\caption{Total error of MISC and MC to compute Basket ($2$-dimensional) Call option price of the different tolerances for different number of time steps, without Richardson extrapolation. The numbers between parentheses are the corresponding absolute errors.}
...
Method \textbackslash  Steps            & $2$ & $4$ & $8$ & $16$ &   \\
\hline
MISC ($TOl=5.10^{-1}$) & $3$ & $15$ & $197$ & $\red{ 1048}$  \\
MISC ($TOl=10^{-1}$)  & $3$ & $34$ & $\red{217}$ & $ 7324$  \\
MISC ($TOl=5.10^{-2}$)  & $3$ & $\red{44}$ & $917$ & $-$  \\
MISC ($TOl=10^{-2}$)  & $\red{4}$ & $98$ & $1825$ & $-$  \\
MISC ($TOl=10^{-3}$)   & $4$ & $245$ & $-$ & $-$  \\
MISC ($TOl=10^{-4}$)   & $23$ & $1035$ & $-$ & $-$  \\
MISC ($TOl=10^{-5}$)   & $58$ & $-$ & $-$ & $-$  \\
\hline
...
\hline
\end{tabular}
\caption{Comparison of the computational time of  MC and MISC, used to compute Basket ($2$-dimensional) Call option price  for different number of time steps, without Richardson extrapolation}
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		...
		\hline
		MISC ($TOl=5.10^{-1}$, $\beta=16$) & $   13.0821
		$ & $ 12.7353
		$ & $   12.8138$   \\
		MISC ($TOl=10^{-1}$, $\beta=16$)  & $    13.1898
		$ & $ 12.8791
		$ & $   12.9055$  \\
		MISC ($TOl=5.10^{-2}$, $\beta=16$) & $    13.1898
		$ & $ 12.9151 $ & $-$   \\
		MISC ($TOl=10^{-2}$,$\beta=16$ ) & $    13.1898
		$ & $ 
		12.9567$ & $-$  \\
		MISC ($TOl=10^{-3}$, $\beta=16$) & $    13.1898
		$ & $ 12.9558$ & $-$ \\
			MISC ($TOl=10^{-4}$,$\beta=16$) & $  13.1893
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		Method \textbackslash  Steps            & $1-2$ & $2-4$ & $4-8$   \\
		\hline
		MISC ($TOl=5.10^{-1}$)  & $\underset{(0.1106)}{\mathbf{0.0086
		}}$ & $\underset{(0.2415)}{\mathbf{0.0187}}$& $\underset{(
		    0.1184
		
			)}{\mathbf{ 0.0092}}$  \\
		MISC ($TOl=10^{-1}$)  & $\underset{(0.0029)}{\mathbf{\red{0.0002}
		}}$  & $\underset{(     0.0977
	 )}{\mathbf{      0.0076}}$& $\underset{(
		    0.0267
		
			)}{\mathbf{   \red{0.0021}}}$  \\
		MISC ($TOl=5.10^{-2}$) & $\underset{(0.0029)}{\mathbf{0.0002
		}}$ & $\underset{(    0.0617)}{\mathbf{  0.0048}}$& $\underset{(
			-
			)}{\mathbf{-}}$  \\
		MISC ($TOl=10^{-2}$)  & $\underset{(0.0029)}{\mathbf{0.0002
		}}$ & $\underset{(  0.0201)}{\mathbf{   \red{0.0016}}}$& $\underset{(
			-
			)}{\mathbf{-}}$ \\
		MISC ($TOl=10^{-3}$)  & $\underset{(0.0029)}{\mathbf{0.0002
		}}$  & $\underset{(    0.0210
	)}{\mathbf{   0.0016}}$& $\underset{(
			-
			)}{\mathbf{-}}$  \\
		MISC ($TOl=10^{-4}$)  & $\underset{(0.0029)}{\mathbf{0.0002
		}}$ & $\underset{(-)}{\mathbf{-}}$& $\underset{(
			-
			)}{\mathbf{-}}$  \\
		...
		\hline
	\end{tabular}
	\caption{Quadrature error of MISC to compute Basket ($2$-dimensional) call option price of the different tolerances for different number of time steps, with Richardson extrapolation. The numbers between parentheses are the corresponding absolute errors.}
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		Method \textbackslash  Steps            & $1-2$ & $2-4$ & $4-8$   \\
		\hline
		MISC ($TOl=5.10^{-1}$) & $\mathbf{0.0312}$& $\mathbf{0.0246}$ & $\mathbf{0.0116}$  \\
		MISC ($TOl=10^{-1}$) & $\mathbf{\red{0.0228}}$& $\mathbf{0.0135}$ & $\mathbf{\red{0.0045}}$   \\	
		MISC ($TOl=5.10^{-2}$) & $\mathbf{0.0228}$& $\mathbf{0.0107}$ & $\mathbf{-}$   \\
		MISC ($TOl=10^{-2}$)& $\mathbf{0.0228}$& $\mathbf{\red{0.0075}}$ & $\mathbf{-}$   \\	
		MISC ($TOl=10^{-3}$) & $\mathbf{0.0228}$& $\mathbf{0.0075}$ & $\mathbf{-}$ \\
		MISC ($TOl=10^{-4}$) & $\mathbf{0.0228}$& $\mathbf{-}$ & $\mathbf{-}$  \\
		\hline	
		MC +root finding & $\mathbf{\red{-}}$ & $\mathbf{\red{-}}$ & $\mathbf{\red{-}}$   \\	
		MC   &  $\mathbf{\red{-}}$ & $\mathbf{\red{-}}$ & $\mathbf{\red{-}}$   \\	
		\hline
	\end{tabular}
	\caption{Total relative error of MISC and MC to compute Basket ($2$-dimensional) call option price of the different tolerances for different number of time steps, with Richardson extrapolation. The numbers between parentheses are the corresponding absolute errors.}
...
\begin{table}[h!]
	...
	\begin{tabular}{l*{6}{c}r}
		Method \textbackslash  Steps            & $1-2$ & $2-4$ & $4-8$     \\
		\hline
		MISC ($TOl=5.10^{-1}$) & $5$  & $39$ & $396$  \\
		MISC ($TOl=10^{-1}$)  & $\red{8}$  & $70$ & $ \red{1379}$  \\
		MISC ($TOl=5.10^{-2}$)  & $8$  & $76$ & $-$   \\
		MISC ($TOl=10^{-2}$) & $8$  & $ \red{262}$ & $-$ \\
		MISC ($TOl=10^{-3}$)   & $8$  & $339$ & $-$   \\
		MISC ($TOl=10^{-4}$)   & $55$  & $-$ & $-$   \\
		\hline
		...
		\hline
	\end{tabular}
	\caption{Comparison of the computational time of  MISC, used to compute Basket $2$-dimensional Call option price  for different number of time steps, with Richardson extrapolation (level $1$)}
