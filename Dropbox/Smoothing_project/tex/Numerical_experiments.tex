...
\end{figure}
\FloatBarrier
We had some issues with the convergence of adaptive sparse grids (slow and unstable), and by investigating the first and mixed differences rates (see Figure \ref{fig:first_second_diff_Heston_call}  for the case of $N=2$ but we mention that we observed similar behavior for higher number of time steps $N$), it seems that there is an issue with dimensions related to the volatility noise which I am still working on it to detect the source of the issue and fix it to have a better behavior.
\FloatBarrier
\begin{figure}[h!]
	\centering
	\begin{subfigure}{.4\textwidth}
