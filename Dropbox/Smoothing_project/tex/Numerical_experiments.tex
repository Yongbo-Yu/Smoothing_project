...
From figure \ref{fig:Weak_rate_binary}, we see that we get a weak error of order $\Delta t$ for the case without Richardson extrapolation. We note that the weak errors plotted here correspond to relative errors.  The upper and lower bounds are $95\%$ confidence interval.
...
%	\end{subfigure}
	...
	\caption{The rate of convergence of the weak error for the binary option, using $M=10^4$ samples for MC.}
...
\begin{figure}[h!]
	...
	\caption{The rate of convergence of the weak error for the two dimensional basket call option with  a number of Laguerre  quadrature points  $\beta=128$ and number of samples for MC $M=10^7$.  }
...
\begin{table}[h!]
	...
	\hline
           & $2$ & $4$ & $8$   \\
		\hline
		MISC   &  $\underset{(0.01,0.006)}{\mathbf{}}$ & $\underset{(0.0079,0.0055)}{\mathbf{0.0134}}$ & $\underset{(0.0043,0.0042)}{\mathbf{0.0085}}$   \\
...
\subsection{The best call option  under GBM and Heston model}\label{sec:The best call option  under GBM and Heston model}
