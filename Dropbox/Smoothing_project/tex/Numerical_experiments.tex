...
The  first example that we test under the Heston model is the single European call option with parameters: $S_0=K=100$, $v_0=0.04$, $\mu=0$, $\kappa=5$, $\theta=0.04$, $\xi=0.5$ and $\rho=-0.9$. The reference value in this case is $7.5789$. For solving the root finding problem we follow similar approach to the one in Section \ref{sec:Determining the kink location} with substituting $\sigma$ with $\sqrt{v_i}$ in $f_i$.
...
\end{figure}
\FloatBarrier
We had some issues with the convergence of adaptive sparse grids (slow and unstable), that is why, I tested two cases:
\begin{itemize}
\item The first case where all coordinates (corresponding to asset and volatility noise) are constructed using hierarchical Brownian bridge. In this case, we had the worst performance (see Figure \ref{fig:first_second_diff_Heston_call}) and this is may be to the behavior of the integrand with respect  to the coordinates related to the volatility noise (see Figure \ref{fig:integrand behaviour with respect to the different coordinates_1D_hierarchical}).
\item  In the second case, I use just hierarchical Brownian bridge for the coordinates used for the asset dynamics. I had a better behavior in terms of first and second differences behavior  (see Figure \ref{fig:first_second_diff_Heston_call_non_hierarchical}) however there are still some slow directions, which affect the speed of the adaptive sparse grids. By looking at the the behavior of the integrand with respect  to the coordinates related to the volatility noise (see Figure \ref{fig:integrand behaviour with respect to the different coordinates_1D_non_hierarchical}), we see better behavior than Figure \ref{fig:integrand behaviour with respect to the different coordinates_1D_hierarchical} but it seems from Figure \ref{fig:integrand behaviour with respect to the different coordinates_1D_non_hierarchical} that maybe using Gauss-Laguerre quadrature for those coordinates will be the optimal choice rather than Gauss-Hermite as it for now.
\end{itemize}
 
\subsubsection*{All hierarchical}
By investigating the first and mixed differences rates (see Figure \ref{fig:first_second_diff_Heston_call}  for the case of $N=2$ but we mention that we observed similar behavior for higher number of time steps $N$), it seems that there is an issue with dimensions related to the volatility noise.
\FloatBarrier
\begin{figure}[h!]
	\centering
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/Heston_single_call/mixed_rates/all_hierarchical/first_difference_heston_2steps_all_hierarchicial}
		\caption{}
		\label{fig:sub3}
	\end{subfigure}%
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/Heston_single_call/mixed_rates/all_hierarchical/mixed_difference_order2_heston_2steps}
		\caption{}
		\label{fig:sub4}
	\end{subfigure}
	\caption{The rate of error convergence of first and second order differences $\abs{\Delta \text{E}_{\boldsymbol{\beta}}}$, defined by \eqref{eq:Work_error_contributions}, ($\boldsymbol{\beta}=\mathbf{1}+k \bar{\boldsymbol{\beta}}$) for the example of single call option under Heston model using $N=2$ time steps. In this case, the first two dimensions are used for the volatility noise (mainly $Z_v$ in \eqref{eq:FE_Heston_discrete}) and the final coordinate is related to $Z_t$ (used to get the noise related to the asset dynamics) (a) first order differences (b) second order differences. }
	\label{fig:first_second_diff_Heston_call}
\end{figure}
\FloatBarrier
...
\begin{figure}[!h]
	...
	\begin{subfigure}[b]{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/Heston_single_call/integrand_plotting/hierarchical/1D/heston_2steps_hierarchical_y1}
		\caption{}
	\end{subfigure}%
	\begin{subfigure}[b]{.4\textwidth}
		\centering
		\includegraphics[width=1.\linewidth]{./figures/Heston_single_call/integrand_plotting/hierarchical/1D/heston_2steps_hierarchical_y2}
		\caption{}
	\end{subfigure}\\[1ex]
	\begin{subfigure}{.4\textwidth}
		\centering
			\includegraphics[width=1.\linewidth]{./figures/Heston_single_call/integrand_plotting/hierarchical/1D/heston_2steps_hierarchical_y3}
		\caption{}
		\end{subfigure}%
	\caption{The integrand behaviour with respect to the different coordinates ($(y_1,y_2)$ are used for the volatility noise (mainly $Z_v$ in \eqref{eq:FE_Heston_discrete}) and the final coordinate $y_3$ is related to $Z_t$ (used to get the noise related to the asset dynamics) :  (a) with respect to $y_1$, b) with respect to $y_2$, c) with respect to $y_3$.}
	\label{fig:integrand behaviour with respect to the different coordinates_1D_hierarchical}
\end{figure}
\FloatBarrier
\subsubsection*{Partial hierarchical}
...
\begin{figure}[h!]
	...
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/Heston_single_call/mixed_rates/non_hierarchical/first_difference_heston_2steps_non_hierarchical}
		\caption{}
		\label{fig:sub3}
	\end{subfigure}%
	\begin{subfigure}{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/Heston_single_call/mixed_rates/non_hierarchical/mixed_difference_order2_heston_2steps_non_hierarchical}
		\caption{}
		\label{fig:sub4}
	\end{subfigure}
	\caption{The rate of error convergence of first and second order differences $\abs{\Delta \text{E}_{\boldsymbol{\beta}}}$, defined by \eqref{eq:Work_error_contributions}, ($\boldsymbol{\beta}=\mathbf{1}+k \bar{\boldsymbol{\beta}}$) for the example of single call option under Heston model using $N=2$ time steps. In this case, the first two dimensions are used for the volatility noise (mainly $Z_v$ in \eqref{eq:FE_Heston_discrete}) and the final coordinate is related to $Z_t$ (used to get the noise related to the asset dynamics) (a) first order differences (b) second order differences. }
	\label{fig:first_second_diff_Heston_call_non_hierarchical}
\end{figure}
\FloatBarrier
...
\begin{figure}[!h]
	...
	\begin{subfigure}[b]{.4\textwidth}
		\centering
		\includegraphics[width=1\linewidth]{./figures/Heston_single_call/integrand_plotting/non_hierarchical/1D/heston_2steps_non_hierarchical_y1}
		\caption{}
	\end{subfigure}%
	\begin{subfigure}[b]{.4\textwidth}
		\centering
		\includegraphics[width=1.\linewidth]{./figures/Heston_single_call/integrand_plotting/non_hierarchical/1D/heston_2steps_non_hierarchical_y2}
		\caption{}
	\end{subfigure}\\[1ex]
	\begin{subfigure}{.4\textwidth}
		\centering
			\includegraphics[width=1.\linewidth]{./figures/Heston_single_call/integrand_plotting/non_hierarchical/1D/heston_2steps_non_hierarchical_y3}
		\caption{}
		\end{subfigure}%
	\caption{The integrand behaviour with respect to the different coordinates ($(y_1,y_2)$ are used for the volatility noise (mainly $Z_v$ in \eqref{eq:FE_Heston_discrete}) and the final coordinate $y_3$ is related to $Z_t$ (used to get the noise related to the asset dynamics) :  (a) with respect to $y_1$, b) with respect to $y_2$, c) with respect to $y_3$.}
	\label{fig:integrand behaviour with respect to the different coordinates_1D_non_hierarchical}
\end{figure}
\FloatBarrier

\subsubsection*{Conclusion}
We think that observing these behaviors for first and mixed differences is due to the non smooth transformation we applied to guarantee the positiveness of the CIR process, in particular the $f_2$  and $f_3$ functions used in the full truncation scheme, which somehow deteriorates the overall smoothness of the Heston process. As a remedy to this we suggest to use a different way to simulate the volatility process. In fact,  we use the idea that  any CIR process can be defined as a sum of squared Ornstein-Uhlenbeck processes. Please see below section for details.

\subsubsection*{Relation between CIR process and Ornstein-Uhlenbeck process}
It is well known that any Ornstein-Uhlenbeck (OU) process is normally distributed. Thus the sum of $n$ squared OU processes is chi-squared distributed with $n$ degrees of freedom. Let us define $\mathbf{X}$ to be a $n$-dimensional vector valued OU process with
...
\subsubsection*{How we can adapt the previous result to our case}
We think that we can use the results of the previous section, and simulate the  OU process  $\mathbf{X}$, with dynamics given by \eqref{equivalent OU process} such that its parameters $(\alpha, \beta)$ are expressed in terms of  the volatility  process  parameters, that is 
$$ \alpha=-\frac{\kappa}{2},\quad \beta=\frac{\xi}{2}, \quad n=\frac{-2 \theta \alpha}{\beta^2}=\frac{4 \theta  \kappa}{\xi^2}$$

For instance, with the example parameters we are using, we end up with $\alpha=-2.5$, $\beta=0.25$ and $n=3.2$.
...
\begin{remark}
Raul and I discussed the case where we consider $n=1$ and work with it, but after looking again I think this is not feasible since the identification system becomes ill-defined.
\end{remark} 
