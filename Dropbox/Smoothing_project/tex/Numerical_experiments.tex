...
\begin{table}[!h]
	...
	\toprule[1.5pt]
		...
		\hline
			  Single binary option (GBM) & without  &  $2 \%$&  $ 3.6$ \\	
              \hline
            Single call option (GBM)  & without    &  $0.9\%$&  $22.4$ \\
%            & level $1$ &  $$ &  $$ \\		
				 \hline
					Basket call option (GBM)  & without    &  $$&  $$ \\	
					\hline
...
\end{align}
wehre $(z_1,\dots,z_N)$ are standard Gaussian random variables, for some path function $\Phi$ and Brownian bridge map $\Psi$. As explained in Sections \ref{sec:Discrete time, practical motivation} and \ref{sec:Details of our approach}, the first step of our approach is determining the location of irregularity (kink). In the following, we want to compare different ways for identifying the location of the kink for this model.
...
\begin{align}\label{smoothed_integrand_single_opt_1d}
	h(\mathbf{z}_{-1})&= \int_{\rset}  g   \left( \Phi \circ \Psi(T;z_1,\mathbf{z}_{-1})\right)   \rho_1(z_1) dz_1.
\end{align}
...
\begin{align}
	h(\mathbf{z}_{-1}) &:=  \int_{\rset} g \left(\Phi \circ \Psi(T;z_1,\mathbf{z}_{-1})\right)   \rho_1(z_1) dz_1 \nonumber\\
	&=\int_{-\infty}^{y^\ast} g \left( \Phi \circ \Psi(T;z_1,\mathbf{z}_{-1})\right)  \rho_d(z_1) dz_1 +\int_{y^\ast}^{\infty} g \left( \Phi \circ \Psi(T;z_1,\mathbf{z}_{-1})\right) \rho_d(z_1) dz_1,
\end{align}
...
\begin{align}\label{smoothed_integrand_binary_opt_2}
	h(\mathbf{z}_{-1})&= \int_\rset \mathbf{1}_{\Phi \circ \Psi(T;z_1,\mathbf{z}_{-1})>K}\frac{1}{\sqrt{2 \pi}} \operatorname{exp} (-z_1^2/2) dy \nonumber\\
	&=  P(Y>y_{\ast}(K)),
\end{align}
where  $y_{\ast}(x)$, is an invertible function that satisfies 
\begin{align}
	\Phi \circ \Psi (T;y_{\ast}(x),\mathbf{z}_{-1})=x	 \PERIOD
\end{align}
...
From figure \ref{fig:Weak_rate_binary}, we see that we get a weak error of order $\Delta t$ for the case without Richardson. We note that the weak errors plotted here correspond to relative errors.  The upper and lower bounds are $95\%$ confidence interval.
...
%	\end{subfigure}
	...
	\caption{The rate of convergence of the weak error for the binary option.  }
...
%		MISC ($TOL_{\text{MISC}}=10^{-3}$)   & $\underset{(0.04, 0.01)}{\mathbf{ 0.05}}$  & $\underset{(0.02,0.002)}{\mathbf{ 0.02}}$  & $\underset{(0.013,0.002)}{\mathbf{\red{0.015}}}$\\
		...
		MC+root finding   &   $\underset{(0.04,0.04 )}{\mathbf{0.08}}$ & $\underset{(0.02,0.02)}{\mathbf{0.04}}$ & $\underset{(0.01,0.01)}{\mathbf{0.02}}$  \\	
			M(\# MC samples)   & $\times 10^1$  & $4 \times 10^1$  & $  10^2$ \\	
		\hline
...
h(\mathbf{z}_{-1})&= \int_{\rset}  \max \left(\Phi \circ \Psi(T;z_1,\mathbf{z}_{-1})-K,0\right) \frac{1}{\sqrt{2 \pi}} \operatorname{exp}(-z_1^2/2) dz_1 \PERIOD
\end{align}
...
\begin{align}
	h(\mathbf{z}_{-1}) &:=  \int_{\rset} \max \left(\Phi \circ \Psi(T;z_1,\mathbf{z}_{-1})-K,0\right) \frac{1}{\sqrt{2 \pi}} \operatorname{exp}(-z_1^2/2) dy \\ \nonumber
	&=\int_{-\infty}^{y^{\ast}} \max \left(\Phi \circ \Psi(T;z_1,\mathbf{z}_{-1})-K,0\right) \frac{1}{\sqrt{2 \pi}} \operatorname{exp}(-z_1^2/2) dz_1\\ \nonumber
	&+\int_{y^{\ast}}^{\infty} \max \left(\Phi \circ \Psi(T;z_1,\mathbf{z}_{-1})-K,0\right) \frac{1}{\sqrt{2 \pi}} \operatorname{exp}(-z_1^2/2) dz_1,
\end{align}
...
We can see from figure \ref{fig:Weak_rate_call_beta_32} that we get a weak error of order $\Delta t$ for the case without Richardson extrapolation.  The upper and lower bounds are $95\%$ confidence interval.
...
%	\end{subfigure}
	...
	\caption{The rate of convergence of the weak error for the call option using MC.}
...
\begin{table}[h!]
	...
	\hline
           & $2$ & $4$ & $8$ & $16$  \\
		\hline
		MISC ($TOL_{\text{MISC}}=5.10^{-1}$)  &  $\underset{(0.021,0.001)}{\mathbf{0.022}}$ & $\underset{(0.016,0.002)}{\mathbf{0.018}}$ & $\underset{(0.009,0.0001)}{\mathbf{0.009}}$ & $\underset{(0.004,0.0004)}{\mathbf{0.004}}$  \\
...
\begin{table}[h!]
	...
	\hline
	         & $2$ & $4$ & $8$ & $16$ &   \\
		\hline
		MISC ($TOL_{\text{MISC}}=5.10^{-1}$) & $0.3$ & $3$ & $17$ & $473$  \\
...
%		MC method ($M=10^5, \beta=32$)    & $168$ & $216$ & $290$ & $\red{432}$  \\
			MC method +root finding   & $3$ & $16$ & $70$ & $408$  \\
...
\subsection{The basket call option  under GBM  model}
The third example that we consider  is the multi-dimensional  basket call option under GBM. 



\subsection{The best call option  under GBM and Heston model}
The fourth example that we consider is the multi-dimensional  best call option under GBM and Heston model. I am in process of formulating this but somehow we agreed that the first potential directions of smoothing for the Heston model will be the first factors related to the asset prices.
